...
Indeed, solving the QP can be seen as computing a step to modify an initial-guess $\dxtrajguess$, $\dutrajguess$. Such an observation is somehow trivial, as for a QP, any initial guess will find in a single step the exact same optimum.
...
In such a case the flow $V_t$ would be typically computed at $\dx_t$, while the Jacobians $F_x$, $F_u$ are computed at the next state, i.e. $\dx_{t-1}^+ = F_x \dx + F_u \du + f_{t-1}$.
...
This term is never mentioned in DDP equations as far as we know.
...
Yet, the DDP classically observes that a feasible solution can ne directly obtained by integrating the nonlinear dynamics around a candidate solution $\utraj$ from the initial state $x_0$.
