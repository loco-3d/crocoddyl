...
oc = import_control_example("quadrotor_fwddyn")

model = oc.hector.model.copy()
state = crocoddyl.StateMultibody(model)

joint_dynamics = [crocoddyl.JointDynamicsModelThruster(oc.ps)]
...
for i in range(nbodies):
    ...
    param.toParametrization(initial_p[s], 0.7 * psi)
...
print("*** quadrotor inertial parameter estimation (invdyn) ***")
solver.solve(init_xs=list(oc.solver.xs), init_us=[], init_p=[initial_p], maxiter=100)

p_est = solver.p[0]
estimated_total_mass = 0.0
...
for i in range(nbodies):
    ...
    estimated_total_mass += psi_est[0]
    print(f"  body {i + 1}: mass est={psi_est[0]:.4f}  gt={psi_gt[0]:.4f}")
print("total-mass error:", abs(original_total_mass - estimated_total_mass))
