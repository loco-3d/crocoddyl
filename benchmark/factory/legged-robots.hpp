...
void build_contact_action_models(
  ...
  FramePlacement Mref(
      model.getFrameId(robotNames.ee_name),
  ...
  boost::shared_ptr<CostModelAbstract> xRegCost =
      boost::make_shared<CostModelState>(state, default_state,
                                         actuation->get_nu());
  ...
  for (std::size_t i = 0; i < robotNames.contact_names.size(); ++i) {
    ...
    case Contact3D: {
      FrameTranslation contact_ref(
          model.getFrameId(robotNames.contact_names[i]),
          Eigen::Vector3d::Zero());
      boost::shared_ptr<ContactModelAbstract> support_contact =
          boost::make_shared<ContactModel3D>(state, contact_ref,
                                             actuation->get_nu(),
