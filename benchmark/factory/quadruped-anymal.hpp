...
#ifndef CROCODDYL_QUADRUPED_ANYMAL_FACTORY_HPP_
#define CROCODDYL_QUADRUPED_ANYMAL_FACTORY_HPP_
...
void build_quadruped_anymal_action_models(boost::shared_ptr<crocoddyl::ActionModelAbstractTpl<Scalar> >& runningModel,
  ...
  const std::string RF = "RF_KFE";
  const std::string LF = "LF_KFE";
  const std::string LH = "LH_KFE";

  pinocchio::ModelTpl<double> modeld;
  pinocchio::urdf::buildModel(EXAMPLE_ROBOT_DATA_MODEL_DIR "/anymal_b_simple_description/robots/anymal.urdf",
                              pinocchio::JointModelFreeFlyer(), modeld);
  modeld.lowerPositionLimit.head<7>().array() = -1;
  modeld.upperPositionLimit.head<7>().array() = 1.;
  pinocchio::srdf::loadReferenceConfigurations(modeld, EXAMPLE_ROBOT_DATA_MODEL_DIR "/anymal_b_simple_description/srdf/anymal.srdf",
                                               false);
  ...
  default_state << model.referenceConfigurations["standing"],
  ...
  FramePlacement Mref(model.getFrameId("RH_KFE"),
  ...
  RigidContactModel ci_LH(pinocchio::CONTACT_3D,model.getFrameId(LH),pinocchio::LOCAL);

  contact_models.push_back(ci_RF);
  contact_models.push_back(ci_LF);
  contact_models.push_back(ci_LH);
