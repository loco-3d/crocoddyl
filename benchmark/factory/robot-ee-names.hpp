...
struct RobotEENames {
  RobotEENames(const std::string &robot_name,
      ...
      : robot_name(robot_name), contact_names(contact_names),
        contact_types(contact_types), urdf_path(urdf_path),
        srdf_path(srdf_path), ee_name(ee_name), reference_conf(reference_conf) {
  }
