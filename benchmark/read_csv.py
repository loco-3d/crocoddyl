...
def parseCsvFile(filename, sep=',', delimiter=None):
    ...
    """
    col_types = dict(fn_name=str,nthreads=int,with_cg=bool,
                     mean=float,stddev=float,max=float,min=float,
                     mean_per_nodes=float,stddev_per_nodes=float)
    
    seq = pd.read_csv(filename, header=0,
                      sep=sep, delim_whitespace=False,
                      quoting = 2, index_col=0, dtype=col_types)
