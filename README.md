...
The Python examples are organized into `control`, `estimation`, and `timeopt`
groups. After installation, you can run a control example as follows:
```bash
python -m crocoddyl.examples.control.quadruped_gaits_fwddyn "display" "plot"
```
From the build directory, each example has a target named
`run_examples.<group>.<name>`:
```bash
cd build
make test
make -s run_examples.control.quadruped_gaits_fwddyn INPUT="display plot"
make -s run_examples.estimation.double_pendulum_inertialparam_fwddyn
make -s run_examples.timeopt.double_pendulum_timeopt_fwddyn
make -s benchmarks-cpp-quadrupedal_gaits INPUT="100 walk" # number of trials ; type of gait
```
You can also run a complete group, or every example group, using the aggregate
targets:
```bash
make run_examples.control
make run_examples.estimation
make run_examples.timeopt
make run_examples
```
