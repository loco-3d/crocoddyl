...
void exposeDifferentialActionAbstract() {
  ...
  bp::class_<DifferentialActionModelAbstract_wrap, boost::noncopyable>(
      ...
      "In crocoddyl, a differential action model combines dynamics and cost "
      "data described in\n"
      "continuous time. Each node, in our optimal control problem, is "
      "described through an\n"
      "action model. Every time that we want describe a problem, we need to "
      "provide ways of\n"
