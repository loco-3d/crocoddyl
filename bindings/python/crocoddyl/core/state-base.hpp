...
public:
  ...
  Eigen::VectorXd
  integrate_wrap(const Eigen::Ref<const Eigen::VectorXd> &x,
  ...
  Eigen::MatrixXd
  JintegrateTransport_wrap(const Eigen::Ref<const Eigen::VectorXd> &x,
                           const Eigen::Ref<const Eigen::VectorXd> &dx,
                           Eigen::Ref<Eigen::MatrixXd> Jin,
                           const Jcomponent firstsecond) const {
    assert_pretty(
    ...
    return bp::call<Eigen::MatrixXd>(
        this->get_override("JintegrateTransport").ptr(), x, dx,
        (Eigen::MatrixXd)Jin, firstsecond);
  }
};
