...
model = SimpleDynamicsModel(DT)
data = model.createData()
com0 = np.matrix([0.]).T
com1 = com0 + np.matrix([1.]).T
...
x0 = np.matrix(np.zeros((model.n,1)))
x0[:] = com0

x1 = np.matrix(np.zeros((model.n,1)))
x1[:] = com1

u = np.matrix(np.zeros((model.m,1)))
...
problem = DDPPhase(model,r1_cost,t_cost,EulerIntegrator,timeline)
