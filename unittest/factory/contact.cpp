...
const std::vector<ContactModelTypes::Type>
    ContactModelTypes::all(ContactModelTypes::init_all());
...
boost::shared_ptr<crocoddyl::ContactModelAbstract> ContactModelFactory::create(
  ...
  case ContactModelTypes::ContactModel2D:
    contact = boost::make_shared<crocoddyl::ContactModel2D>(
        state, crocoddyl::FrameTranslation(frame_id, Eigen::Vector3d::Zero()),
        nu);
    break;
  case ContactModelTypes::ContactModel3D:
...
boost::shared_ptr<crocoddyl::ContactModelAbstract> create_random_contact() {
  ...
  if (rand() % 3 == 0) {
    ...
    contact = factory.create(ContactModelTypes::ContactModel3D,
                             PinocchioModelTypes::RandomHumanoid);
  } else {
    contact = factory.create(ContactModelTypes::ContactModel6D,
                             PinocchioModelTypes::RandomHumanoid);
  }
  return contact;
}
