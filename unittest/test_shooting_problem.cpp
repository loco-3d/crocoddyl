...
class ShootingProblemFactory {
 public:
   ...
   * @param horizon_size
   */
  ShootingProblemFactory(int nx, int nu, bool drift_free, unsigned horizon_size) {
    ...
    x0_.fill(1.0);
    shooting_problem_ = new crocoddyl::ShootingProblem(x0_, running_models_, terminal_model_);
  }
   ...
   */
  crocoddyl::ShootingProblem& get_shooting_problem() { return *shooting_problem_; }
...
void test_trajectory_dimension(int nx, int nu, bool drift_free, unsigned horizon_size) {
  ...
  ShootingProblemFactory<crocoddyl::ActionModelLQR> factory(nx, nu, drift_free, horizon_size);
...
void test_calc() {
  ...
  ShootingProblemFactory<ActionModelLQRIdentity> factory(nx, nu, drift_free, horizon_size);
  crocoddyl::ShootingProblem& shooting_problem = factory.get_shooting_problem();
  ...
  for (unsigned i = 0; i < u_vec.size(); ++i) {
    u_vec[i].fill(2.0);
  }
  ...
  double tol = std::sqrt(2.0 * std::numeric_limits<double>::epsilon());
  BOOST_CHECK((shooting_problem.get_runningDatas()[0]->xnext - (Eigen::VectorXd(2) << 3, 1).finished())
                  .isMuchSmallerThan(1.0, tol));
  BOOST_CHECK((shooting_problem.get_runningDatas()[1]->xnext - (Eigen::VectorXd(2) << 3, 1).finished())
                  .isMuchSmallerThan(1.0, tol));
  BOOST_CHECK((shooting_problem.get_runningDatas()[2]->xnext - (Eigen::VectorXd(2) << 3, 1).finished())
                  .isMuchSmallerThan(1.0, tol));
  BOOST_CHECK((shooting_problem.get_terminalData()->xnext - (Eigen::VectorXd(2) << 1, 1).finished())
                  .isMuchSmallerThan(1.0, tol));
}
...
void test_calc_diff(int nx, int nu, bool drift_free, unsigned horizon_size) {
  ...
  ShootingProblemFactory<crocoddyl::ActionModelLQR> factory(nx, nu, drift_free, horizon_size);
  ShootingProblemFactory<crocoddyl::ActionModelLQR> factory2(nx, nu, drift_free, horizon_size);
  crocoddyl::ShootingProblem& shooting_problem = factory.get_shooting_problem();
  ...
  BOOST_CHECK(shooting_problem.get_runningDatas()[0]->cost == shooting_problem2.get_runningDatas()[0]->cost);
  BOOST_CHECK(shooting_problem.get_runningDatas()[1]->cost == shooting_problem2.get_runningDatas()[1]->cost);
  BOOST_CHECK(shooting_problem.get_runningDatas()[2]->cost == shooting_problem2.get_runningDatas()[2]->cost);
  BOOST_CHECK(shooting_problem.get_terminalData()->cost == shooting_problem2.get_terminalData()->cost);

  double tol = std::sqrt(2.0 * std::numeric_limits<double>::epsilon());
  BOOST_CHECK((shooting_problem.get_runningDatas()[0]->xnext - shooting_problem2.get_runningDatas()[0]->xnext)
                  .isMuchSmallerThan(1.0, tol));
  BOOST_CHECK((shooting_problem.get_runningDatas()[1]->xnext - shooting_problem2.get_runningDatas()[1]->xnext)
                  .isMuchSmallerThan(1.0, tol));
  BOOST_CHECK((shooting_problem.get_runningDatas()[2]->xnext - shooting_problem2.get_runningDatas()[2]->xnext)
                  .isMuchSmallerThan(1.0, tol));
  BOOST_CHECK((shooting_problem.get_terminalData()->xnext - shooting_problem2.get_terminalData()->xnext)
                  .isMuchSmallerThan(1.0, tol));
}
...
void test_rollout() {
  ...
  std::cout << shooting_problem.get_terminalData()->cost << " ; "
            << shooting_problem.get_terminalData()->xnext.transpose() << std::endl;

  BOOST_CHECK(shooting_problem.get_runningDatas()[0]->cost == 9);
  BOOST_CHECK(shooting_problem.get_runningDatas()[1]->cost == 19);
  BOOST_CHECK(shooting_problem.get_runningDatas()[2]->cost == 33);
  BOOST_CHECK(shooting_problem.get_terminalData()->cost == shooting_problem.get_runningDatas().back()->cost);

  double tol = std::sqrt(2.0 * std::numeric_limits<double>::epsilon());
  BOOST_CHECK((shooting_problem.get_runningDatas()[0]->xnext - (Eigen::VectorXd(2) << 3, 1).finished())
                  .isMuchSmallerThan(1.0, tol));
  BOOST_CHECK((shooting_problem.get_runningDatas()[1]->xnext - (Eigen::VectorXd(2) << 5, 1).finished())
                  .isMuchSmallerThan(1.0, tol));
  BOOST_CHECK((shooting_problem.get_runningDatas()[2]->xnext - (Eigen::VectorXd(2) << 7, 1).finished())
                  .isMuchSmallerThan(1.0, tol));
  BOOST_CHECK((shooting_problem.get_terminalData()->xnext - shooting_problem.get_runningDatas().back()->xnext)
                  .isMuchSmallerThan(1.0, tol));
}
...
void register_action_model_lqr_unit_tests() {
  ...
  framework::master_test_suite().add(
      BOOST_TEST_CASE(boost::bind(&test_trajectory_dimension, nx, nu, drift_free, horizon_size)));

  framework::master_test_suite().add(
      BOOST_TEST_CASE(boost::bind(&test_trajectory_dimension, nx, nu, drift_free, horizon_size)));

  framework::master_test_suite().add(BOOST_TEST_CASE(boost::bind(&test_get_T, nx, nu, drift_free, horizon_size)));

  framework::master_test_suite().add(BOOST_TEST_CASE(boost::bind(&test_get_x0, nx, nu, drift_free, horizon_size)));

  framework::master_test_suite().add(BOOST_TEST_CASE(boost::bind(&test_data_dim, nx, nu, drift_free, horizon_size)));

  framework::master_test_suite().add(BOOST_TEST_CASE(&test_calc));

  framework::master_test_suite().add(BOOST_TEST_CASE(boost::bind(&test_calc_diff, nx, nu, drift_free, horizon_size)));

  framework::master_test_suite().add(BOOST_TEST_CASE(&test_rollout));

  framework::master_test_suite().add(BOOST_TEST_CASE(&test_rollout_us));
}
