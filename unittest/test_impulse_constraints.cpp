...
void test_partial_derivatives_against_impulse_numdiff(ImpulseConstraintModelTypes::Type constraint_type,
...
void register_impulse_constraint_model_unit_tests(ImpulseConstraintModelTypes::Type constraint_type,
                                                  PinocchioModelTypes::Type model_type) {
  ...
  ts->add(
      BOOST_TEST_CASE(boost::bind(&test_partial_derivatives_against_impulse_numdiff, constraint_type, model_type)));
  framework::master_test_suite().add(ts);
}

bool init_function() {
  // Test all the impulse constraint model. Note that we can do it only with humanoids
  // as it needs to test the impulse wrench cone
  for (size_t constraint_type = 0; constraint_type < ImpulseConstraintModelTypes::all.size(); ++constraint_type) {
