...
from crocoddyl import SolverBoxDDP, ShootingProblem, SolverDDP, SolverKKT, SolverBoxKKT
from crocoddyl import IntegratedActionModelEuler, DifferentialActionModelLQR, ActionModelLQR
import numpy as np
...
ddpbox.ul = np.array([min(np.minimum(uddp[0],min(us[0]))),]*nu)
ddpbox.uu = np.array([max(np.maximum(uddp[0],max(us[0]))),]*nu)
...
ddpbox.ul = np.array([min(np.minimum(uddp[0],min(us[0]))),]*nu)+1e-3
ddpbox.uu = np.array([max(np.maximum(uddp[0],max(us[0]))),]*nu)-1e-3
...
if limit_lower:
    ...
    ok_range = range(nu); ok_range.pop(limit_id)
...
#BOX DDP VS BOX KKT
xkkt_box,ukkt_box,donekkt_box = boxkkt.solve(maxiter=2, init_xs=xs, init_us=us, qpsolver=quadprogWrapper, ul=ddpbox.ul, uu=ddpbox.uu)
