...
void change_env(
  ...
  crocoddyl::FrameTranslationTpl<Scalar> Tref(
      md->get_pinocchio().getFrameId("gripper_left_joint"), env_vector);
  md->get_costs()
      ->get_costs()
      .find("gripperTrans")
      ->second->cost->set_reference(Tref);
}
...
build_arm_action_model() {
  ...
  typedef typename crocoddyl::CostModelFrameTranslationTpl<Scalar>
      CostModelFrameTranslation;
  typedef typename crocoddyl::CostModelFrameRotationTpl<Scalar>
      CostModelFrameRotation;
  typedef typename crocoddyl::CostModelFrameVelocityTpl<Scalar>
  ...
  boost::shared_ptr<CostModelAbstract> jointLimitCost =
      boost::make_shared<CostModelState>(state, activation_bounded);
...
build_bipedal_action_model() {
  ...
  typedef typename crocoddyl::CostModelStateTpl<Scalar> CostModelState;
  typedef typename crocoddyl::CostModelControlTpl<Scalar> CostModelControl;
  typedef typename crocoddyl::CostModelCoMPositionTpl<Scalar>
      CostModelCoMPosition;
  typedef typename crocoddyl::CostModelContactForceTpl<Scalar>
      CostModelContactForce;
  typedef typename crocoddyl::CostModelCentroidalMomentumTpl<Scalar>
  ...
  boost::shared_ptr<CostModelAbstract> centroidalCost =
      boost::make_shared<CostModelCentroidalMomentum>(state, Vector6s::Zero(),
                                                      actuation->get_nu());
  boost::shared_ptr<CostModelAbstract> comCost =
      boost::make_shared<CostModelCoMPosition>(state, Vector3s::Zero(),
                                               actuation->get_nu());
  boost::shared_ptr<CostModelAbstract> contactForceCost =
      boost::make_shared<CostModelContactForce>(
          state,
          FrameForce(model.getFrameId(RF), pinocchio::ForceTpl<Scalar>::Zero()),
          6, actuation->get_nu());
  boost::shared_ptr<CostModelAbstract> xRegCost =
  ...
  FramePlacement xref(model.getFrameId(RF),
                      pinocchio::SE3Tpl<Scalar>::Identity());
  boost::shared_ptr<ContactModelAbstract> support_contact_model6D =
      boost::make_shared<ContactModel6D>(state, xref, actuation->get_nu(),
  ...
  boost::shared_ptr<ContactModelAbstract> support_contact_model3D =
      boost::make_shared<ContactModel3D>(state, x2ref, actuation->get_nu(),
                                         Vector2s(Scalar(0.), Scalar(50.)));
...
void test_codegen_4DoFArm() {
  ...
  crocoddyl::DifferentialActionModelFreeFwdDynamicsTpl<Scalar> *md =
      static_cast<
          ...
          m->get_differential().get());
  crocoddyl::FrameTranslationTpl<Scalar> Tref(
      md->get_pinocchio().getFrameId("gripper_left_joint"), new_ref);
  md->get_costs()
      ->get_costs()
      .find("gripperTrans")
      ->second->cost->set_reference(Tref);
