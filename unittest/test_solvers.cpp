...
void test_solver_against_kkt_solver(SolverTypes::Type solver_type,
  ...
  // initial state
  BOOST_CHECK(
      (solver->get_xs()[0] - problem->get_x0()).isMuchSmallerThan(1.0, 1e-9));
  ...
  for (unsigned int t = 0; t < T; ++t) {
    ...
    BOOST_CHECK((state->diff_dx(solver->get_xs()[t], kkt.get_xs()[t]))
                    .isMuchSmallerThan(1.0, 1e-9));
    BOOST_CHECK((solver->get_us()[t].head(nu) - kkt.get_us()[t])
                    .isMuchSmallerThan(1.0, 1e-9));
  }
  BOOST_CHECK((state->diff_dx(solver->get_xs()[T], kkt.get_xs()[T]))
                  .isMuchSmallerThan(1.0, 1e-9));
...
bool init_function() {
  ...
  for (size_t action_type = 0; action_type < ActionModelTypes::all.size();
       ++action_type) {
    boost::test_tools::output_test_stream test_name;
    test_name << "test_" << ActionModelTypes::all[action_type];
    test_suite *ts = BOOST_TEST_SUITE(test_name.str());
    ts->add(BOOST_TEST_CASE(boost::bind(
        &test_kkt_dimension, ActionModelTypes::all[action_type], T)));
    ts->add(BOOST_TEST_CASE(boost::bind(
        &test_kkt_search_direction, ActionModelTypes::all[action_type], T)));
    framework::master_test_suite().add(ts);
  }

  // We start from 1 as 0 is the kkt solver
  for (size_t solver_type = 1; solver_type < SolverTypes::all.size();
       ++solver_type) {
    for (size_t action_type = 0;
         action_type < ActionModelTypes::ActionModelImpulseFwdDynamics_HyQ;
         ++action_type) {
      boost::test_tools::output_test_stream test_name;
      test_name << "test_" << SolverTypes::all[solver_type] << "_"
                << ActionModelTypes::all[action_type];
      test_suite *ts = BOOST_TEST_SUITE(test_name.str());
      std::cout << "Running " << test_name.str() << std::endl;
      ts->add(BOOST_TEST_CASE(boost::bind(
          &test_solver_against_kkt_solver, SolverTypes::all[solver_type],
