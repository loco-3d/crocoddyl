...
class SolverBoxKKT(SolverKKT):
    ...
    """
    def solve(self,maxiter=100,init_xs=None,init_us=None,isFeasible=False,
              qpsolver=None, ul=None, uu=None):
    ...
    def computePrimalDual(self):
        ...
        if ul is None and uu is None:
            ...
            qp_C = np.hstack([np.zeros((2*nu, ndx)), np.vstack([np.identity(nu),
                                                                -np.identity(nu)])])
