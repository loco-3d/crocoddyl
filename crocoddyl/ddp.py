...
class SolverDDP:
    ...
    The solver computes an optimal trajectory and control commands
    by iteratively running backward and forward passes. The backward-pass
    updates locally the quadratic approximation of the problem and computes
    descent direction, and the forward-pass rollouts this new policy by
    integrating the system dynamics along a tuple of optimized control
    commands U*.
    ...
    def setCandidate(self, xs=None, us=None, isFeasible=False, copy=True):
        ...
        :params isFeasible: True for xs are obtained from integrating the us
                            (rollout).
    ...
    def solve(self, maxiter=100, init_xs=None, init_us=None,
              isFeasible=False, regInit=None):
        ...
        for i in range(maxiter):
            ...
            for a in self.alphas:
                ...
                if d1 < self.th_grad or not self.isFeasible or \
                   self.dV > self.th_acceptStep * self.dV_exp:
                    # Accept step
                    self.wasFeasible = self.isFeasible
                    self.setCandidate(self.xs_try, self.us_try,
                                      isFeasible=True, copy=False)
    ...
    def allocate(self):
        ...
        self.Q = [np.zeros([m.ndx+m.nu, m.ndx+m.nu])
                  for m in self.problem.runningModels]
        self.q = [np.zeros([m.ndx+m.nu]) for m in self.problem.runningModels]
        self.Qxx = [Q[:m.ndx, :m.ndx]
                    for m, Q in zip(self.problem.runningModels, self.Q)]
        self.Qxu = [Q[:m.ndx, m.ndx:]
                    for m, Q in zip(self.problem.runningModels, self.Q)]
        self.Qux = [Qxu.T
                    for m, Qxu in zip(self.problem.runningModels, self.Qxu)]
        self.Quu = [Q[m.ndx:, m.ndx:]
                    for m, Q in zip(self.problem.runningModels, self.Q)]
        self.Qx = [q[:m.ndx]
                   for m, q in zip(self.problem.runningModels, self.q)]
        self.Qu = [q[m.ndx:]
    ...
    def backwardPass(self):
        ...
        feedforward and feedback commands (i.e. control policy). A
        regularization scheme is used to ensure a good search direction. The
        norm of the gradient, a the directional derivatives are computed.
        """
        xs = self.xs
        ...
        for t, (model, data) in rev_enumerate(zip(self.problem.runningModels,
                                                  self.problem.runningDatas)):
            self.Qxx[t][:, :] = data.Lxx + \
                np.dot(data.Fx.T, np.dot(self.Vxx[t+1], data.Fx))
            self.Qxu[t][:, :] = data.Lxu + \
                np.dot(data.Fx.T, np.dot(self.Vxx[t+1], data.Fu))
            self.Quu[t][:, :] = data.Luu + \
                np.dot(data.Fu.T, np.dot(self.Vxx[t+1], data.Fu))
            self.Qx[t][:] = data.Lx + np.dot(self.Vx[t+1], data.Fx)
            self.Qu[t][:] = data.Lu + np.dot(self.Vx[t+1], data.Fu)
            if not self.isFeasible:
                # In case the xt+1 are not f(xt,ut) i.e warm start not
                # obtained from roll-out.
                relinearization = np.dot(self.Vxx[t+1],
                                         model.State.diff(xs[t+1], data.xnext))
    ...
    def forwardPass(self, stepLength, b=None, warning='ignore'):
        ...
        reasonable improvement. For that we use Armijo condition to evaluated
        the chosen step length.
        :param stepLenght: step length
        """
        # Argument b is introduce for debug purpose.
        # Argument warning is also introduce for debug: by default, it masks
        # the numpy warnings that can be reactivated during debug.
        if b is None:
            b = 1
        ...
        ctry = 0
        for t, (m, d) in enumerate(zip(self.problem.runningModels,
                                       self.problem.runningDatas)):
        ...
        with np.warnings.catch_warnings():
            np.warnings.simplefilter(warning)
            ctry += self.problem.terminalModel.calc(self.problem.terminalData,
                                                    xtry[-1])[1]
