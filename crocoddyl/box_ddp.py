...
class SolverBoxDDP(SolverDDP):
    ...
    def solve(self,maxiter=100,init_xs=None,init_us=None,isFeasible=False,regInit=None, qpsolver=None, ul=None, uu=None):
        self.qpsolver = qpsolver if qpsolver is not None else quadprogWrapper
        self.ul = ul;    self.uu = uu
        return SolverDDP.solve(self,maxiter,init_xs,init_us,isFeasible,regInit)

    def computeGains(self, t):
        uu = self.uu;    ul = self.ul;    u = self.us[t]
        nu = self.problem.runningModels[t].nu
        try:
            if ul is None and uu is None:  qp_d = None;    qp_C = None
            elif uu is not None and ul is None: qp_d = uu-u;   qp_C = np.identity(nu)
            elif uu is None and ul is not None: qp_d = u-ul;   qp_C = -np.identity(nu)
            # qp_d is the opposite sign, because in the implementation of ddp, k and K are
            # the opposite signs.
            else: qp_d = np.hstack([uu-u, u-ul]); qp_C=np.vstack([-np.identity(nu), np.identity(nu)])
            if self.Quu[t].shape[0]>0:
                #We have a -Qu[t] here because in the implementation of ddp,
                #k and K are the negative gains
                sol = self.qpsolver(H=self.Quu[t],
                                    g=-self.Qu[t],C=qp_C,d=qp_d)
                self.k[t][:] = sol.argmin
                free_sel = ~sol.active
