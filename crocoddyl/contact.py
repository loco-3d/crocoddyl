...
class ContactModel3D(ContactModelPinocchio):
    ...
    def calc(model, data, x):
        ...
        data.v = pinocchio.getFrameVelocity(model.pinocchio, data.pinocchio, model.frame).copy()
        vw = data.v.angular
        vv = data.v.linear

        J6 = pinocchio.getFrameJacobian(model.pinocchio, data.pinocchio, model.frame, pinocchio.ReferenceFrame.LOCAL)
        data.J[:, :] = J6[:3, :]
        data.Jw[:, :] = J6[3:, :]
    ...
    def calcDiff(model, data, x, recalc=True):
        ...
        if model.gains[0] != 0.:
            data.Aq[:, :] += model.gains[0] * R * pinocchio.getFrameJacobian(
                model.pinocchio, data.pinocchio, model.frame, pinocchio.ReferenceFrame.LOCAL)[:3, :]
        if model.gains[1] != 0.:
...
class ContactModel6D(ContactModelPinocchio):
    ...
    def calc(model, data, x):
        ...
        if model.gains[1] != 0.:
            data.a0[:] += model.gains[1] * m2a(
                pinocchio.getFrameVelocity(model.pinocchio, data.pinocchio, model.frame).vector)
    ...
    def calcDiff(model, data, x, recalc=True):
        ...
        if model.gains[0] != 0.:
            data.Aq[:, :] += model.gains[0] * np.dot(
                pinocchio.Jlog6(data.rMf),
                pinocchio.getFrameJacobian(model.pinocchio, data.pinocchio, model.frame,
                                           pinocchio.ReferenceFrame.LOCAL))
        if model.gains[1] != 0.:
