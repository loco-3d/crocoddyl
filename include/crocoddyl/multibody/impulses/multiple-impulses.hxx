  ...
  if (static_cast<std::size_t>(df_dx.rows()) != ni_ ||
    ...
    throw_pretty("Invalid argument: "
                 << "df_dx has wrong dimension (it should be " +
                        std::to_string(ni_) + "," + std::to_string(ndx) + ")");
  }
  if (static_cast<std::size_t>(data->impulses.size()) != impulses_.size()) {
    throw_pretty("Invalid argument: "
                 << "it doesn't match the number of impulse datas and models");
...
ImpulseModelMultipleTpl<Scalar>::get_active() const {
  return active_;
}

template <typename Scalar>
const std::vector<std::string> &
ImpulseModelMultipleTpl<Scalar>::get_inactive() const {
  return inactive_;
}
