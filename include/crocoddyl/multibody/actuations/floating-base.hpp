...
class ActuationModelFloatingBaseTpl
 ...
 public:
  ...
  explicit ActuationModelFloatingBaseTpl(
      ...
      : Base(state, state->get_nv() - state->get_pinocchio()->joints[(state->get_pinocchio()->existJointName("root_joint") ? state->get_pinocchio()->getJointId("root_joint") : 0)].nv()){
      };
  ...
  virtual boost::shared_ptr<Data> createData() {
    ...
    const std::size_t root_joint_id = state->get_pinocchio()->existJointName("root_joint") ? state->get_pinocchio()->getJointId("root_joint") : 0;
