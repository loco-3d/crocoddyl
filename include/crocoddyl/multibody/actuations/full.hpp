...
public:
  ...
  explicit ActuationModelFullTpl(boost::shared_ptr<StateMultibody> state)
  ...
  virtual void calcDiff(const boost::shared_ptr<Data> &,
    ...
    assert_pretty(data->dtau_dx ==
                      MatrixXs::Zero(state_->get_nv(), state_->get_ndx()),
                  "dtau_dx has wrong value");
    assert_pretty(data->dtau_du == MatrixXs::Identity(state_->get_nv(), nu_),
