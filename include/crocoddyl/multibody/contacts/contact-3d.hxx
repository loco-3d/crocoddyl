...
template <typename Scalar> ContactModel3DTpl<Scalar>::~ContactModel3DTpl() {}
...
void ContactModel3DTpl<Scalar>::calc(
  ...
  if (gains_[0] != 0.) {
    d->a0 += gains_[0] *
             (d->pinocchio->oMf[xref_.id].translation() - xref_.translation);
  }
  if (gains_[1] != 0.) {
    d->a0 += gains_[1] * d->vv;
  }
...
void ContactModel3DTpl<Scalar>::calcDiff(
  ...
  pinocchio::getJointAccelerationDerivatives(
      *state_->get_pinocchio().get(), *d->pinocchio, d->joint, pinocchio::LOCAL,
  ...
  d->da0_dx.rightCols(nv).noalias() =
      d->fXjda_dv.template topRows<3>() + d->vw_skew * d->Jc -
  ...
  if (gains_[1] != 0.) {
    d->da0_dx.leftCols(nv).noalias() +=
        gains_[1] * d->fXj.template topRows<3>() * d->v_partial_dq;
    d->da0_dx.rightCols(nv).noalias() +=
        gains_[1] * d->fXj.template topRows<3>() * d->a_partial_da;
  }
}
