...
StateMultibodyTpl<Scalar>::StateMultibodyTpl(
    ...
    : Base(model->nq + model->nv, 2 * model->nv), pinocchio_(model),
...
template <typename Scalar> StateMultibodyTpl<Scalar>::~StateMultibodyTpl() {}
...
void StateMultibodyTpl<Scalar>::JintegrateTransport(
  ...
  switch (firstsecond) {
  case first:
    pinocchio::dIntegrateTransport(*pinocchio_.get(), x.head(nq_), dx.head(nv_),
                                   Jin.topRows(nv_), pinocchio::ARG0);
    break;
  case second:
    pinocchio::dIntegrateTransport(*pinocchio_.get(), x.head(nq_), dx.head(nv_),
                                   Jin.topRows(nv_), pinocchio::ARG1);
    break;
  default:
    throw_pretty("Invalid argument: firstsecond must be either first or "
                 "second. both not supported for this operation.");
    break;
  }
}
