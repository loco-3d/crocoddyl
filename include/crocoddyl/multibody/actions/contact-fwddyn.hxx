...
DifferentialActionModelContactFwdDynamicsTpl<Scalar>::
    ...
    : Base(state, actuation->get_nu(), costs->get_nr()), actuation_(actuation),
      contacts_(contacts), costs_(costs),
      pinocchio_(*state->get_pinocchio().get()), with_armature_(true),
      armature_(VectorXs::Zero(state->get_nv())),
      JMinvJt_damping_(fabs(JMinvJt_damping)), enable_force_(enable_force) {
...
void DifferentialActionModelContactFwdDynamicsTpl<Scalar>::quasiStatic(
  ...
  d->pinocchio.tau = pinocchio::rnea(pinocchio_, d->pinocchio, q,
                                     VectorXs::Zero(nv), VectorXs::Zero(nv));
...
template <typename Scalar>
const Scalar &
DifferentialActionModelContactFwdDynamicsTpl<Scalar>::get_damping_factor()
    const {
