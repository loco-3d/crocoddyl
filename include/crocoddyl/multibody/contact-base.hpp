...
template <typename _Scalar> class ContactModelAbstractTpl {
public:
  ...
  void
  setZeroForceDiff(const boost::shared_ptr<ContactDataAbstract> &data) const;
...
template <typename _Scalar> struct ContactDataAbstractTpl {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  ContactDataAbstractTpl(Model<Scalar> *const model,
      ...
      : pinocchio(data), joint(0), frame(0),
        ...
        Jc(model->get_nc(), model->get_state()->get_nv()), a0(model->get_nc()),
