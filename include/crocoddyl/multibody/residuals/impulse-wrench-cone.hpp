...
#ifndef CROCODDYL_MULTIBODY_RESIDUALS_IMPULSE_WRENCH_CONE_HPP_
#define CROCODDYL_MULTIBODY_RESIDUALS_IMPULSE_WRENCH_CONE_HPP_
...
/**
 * @brief Define a impulse wrench cone residual function
 *
 * This residual function is defined as \f$\mathbf{r}=\mathbf{A}\boldsymbol{\lambda}\f$,
 * where \f$\mathbf{A}\f$ is the inequality matrix defined by the impulse wrench cone, and \f$\boldsymbol{\lambda}\f$
 * is the current spatial impulses. The current spatial impulses \f$\boldsymbol{\lambda}\in\mathbb{R}^{nc}\f$ is
 * computed by `ActionModelImpulseFwdDynamicsTpl`, with `nc` as the dimension of the contact.
 *
 ...
 * `ActionModelImpulseFwdDynamicsTpl`, `DataCollectorImpulseTpl`
 */

template <typename _Scalar>
class ResidualModelImpulseWrenchConeTpl : public ResidualModelAbstractTpl<_Scalar> {
 public:
  ...
  typedef ResidualDataImpulseWrenchConeTpl<Scalar> Data;
  ...
  /**
   * @brief Initialize the impulse wrench cone residual model
   *
   * @param[in] state  Multibody state
   * @param[in] id     Reference frame id
   * @param[in] fref   Reference impulse wrench cone
   */
  ResidualModelImpulseWrenchConeTpl(boost::shared_ptr<StateMultibody> state, const pinocchio::FrameIndex id,
                                    const WrenchCone& fref);
  virtual ~ResidualModelImpulseWrenchConeTpl();

  /**
   * @brief Compute the impulse wrench cone residual
   *
   * The force vector is computed by ActionModelImpulseFwdDynamicsTpl and stored in
  ...
  /**
   * @brief Compute the derivatives of the impulse wrench cone residual
   *
   * The force derivatives are computed by ActionModelImpulseFwdDynamicsTpl and stored in
   * DataCollectorImpulseTpl.
   *
   * @param[in] data  Contact force data
   * @param[in] x     State point \f$\mathbf{x}\in\mathbb{R}^{ndx}\f$
  ...
  /**
   * @brief Create the impulse wrench cone residual data
   *
  ...
  /**
   * @brief Return the reference impulse wrench cone
   */
  ...
  /**
   * @brief Modify the reference impulse wrench cone
   */
 ...
 private:
  ...
  WrenchCone fref_;           //!< Reference impulse wrench cone
};

template <typename _Scalar>
struct ResidualDataImpulseWrenchConeTpl : public ResidualDataAbstractTpl<_Scalar> {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  ResidualDataImpulseWrenchConeTpl(Model<Scalar>* const model, DataCollectorAbstract* const data) : Base(model, data) {
...
/* --- Details -------------------------------------------------------------- */
/* --- Details -------------------------------------------------------------- */
#include "crocoddyl/multibody/residuals/impulse-wrench-cone.hxx"

#endif  // CROCODDYL_MULTIBODY_RESIDUALS_IMPULSE_WRENCH_CONE_HPP_
