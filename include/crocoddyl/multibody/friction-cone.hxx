...
template <typename Scalar> FrictionConeTpl<Scalar>::FrictionConeTpl() : nf_(4) {
  A_.resize(nf_ + 1, 3);
  lb_.resize(nf_ + 1);
  ub_.resize(nf_ + 1);
  // compute the matrix
  update(Vector3s(0, 0, 1), Scalar(0.7), true, Scalar(0.),
         std::numeric_limits<Scalar>::max());
}

template <typename Scalar>
FrictionConeTpl<Scalar>::FrictionConeTpl(const Vector3s &normal,
...
FrictionConeTpl<Scalar>::FrictionConeTpl(const FrictionConeTpl<Scalar> &cone)
    : A_(cone.get_A()), lb_(cone.get_lb()), ub_(cone.get_ub()),
      nsurf_(cone.get_nsurf()), mu_(cone.get_mu()), nf_(cone.get_nf()),
      inner_appr_(cone.get_inner_appr()), min_nforce_(cone.get_min_nforce()),
      max_nforce_(cone.get_max_nforce()) {}

template <typename Scalar> FrictionConeTpl<Scalar>::~FrictionConeTpl() {}

template <typename Scalar>
void FrictionConeTpl<Scalar>::update(const Vector3s &normal, const Scalar &mu,
                                     bool inner_appr, const Scalar &min_nforce,
                                     const Scalar &max_nforce) {
  nsurf_ = normal;
  mu_ = mu;
  ...
  Matrix3s c_R_o =
      Quaternions::FromTwoVectors(nsurf_, Vector3s::UnitZ()).toRotationMatrix();
  for (std::size_t i = 0; i < nf_ / 2; ++i) {
    ...
    A_.row(2 * i + 1) =
        (-mu_ * Vector3s::UnitZ() - tsurf_i).transpose() * c_R_o;
...
template <typename Scalar>
const typename MathBaseTpl<Scalar>::MatrixX3s &
FrictionConeTpl<Scalar>::get_A() const {
  return A_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::VectorXs &
FrictionConeTpl<Scalar>::get_lb() const {
  return lb_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::VectorXs &
FrictionConeTpl<Scalar>::get_ub() const {
  return ub_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::Vector3s &
FrictionConeTpl<Scalar>::get_nsurf() const {
  return nsurf_;
}
...
template <typename Scalar> void FrictionConeTpl<Scalar>::set_mu(Scalar mu) {
