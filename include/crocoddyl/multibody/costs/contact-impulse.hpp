 ...
 * reference spatial impulses, respetively. The current spatial impulses
 * \f$\boldsymbol{\lambda}\in\mathbb{R}^{ni}\f$is computed by
 * `DifferentialActionModelContactFwdDynamicsTpl`, with `ni` as the dimension of
 * the impulse.
 *
 ...
 *
 * \sa ActionModelImpulseFwdDynamicsTpl, DataCollectorImpulseTpl,
...
public:
   ...
   * @param[in] fref        Reference spatial contact impulse
   ...
   *
   * We use `ActivationModelQuadTpl` as a default activation model (i.e.
   * \f$a=\frac{1}{2}\|\mathbf{r}\|^2\f$), and `nr` is 6.
   *
   * @param[in] state  Multibody state
   * @param[in] fref   Reference spatial contact impulse
   * \f$\boldsymbol{\lambda}^*\f$
   ...
   *
   * The impulse vector is computed by ActionModelImpulseFwdDynamicsTpl and
   * stored in DataCollectorImpulseTpl.
   *
   ...
   *
   * The impulse derivatives are computed by ActionModelImpulseFwdDynamicsTpl
   * and stored in DataCollectorImpulseTpl.
   *
   * @param[in] data  Contact impulse data
   * @param[in] x     State point \f$\mathbf{x}\in\mathbb{R}^{ndx}\f$
   * @param[in] u     Control input \f$\mathbf{u}\in\mathbb{R}^{nu}\f$
  ...
  DEPRECATED("Used set_reference<FrameForceTpl<Scalar> >()",
             void set_fref(const FrameForce &fref));
  DEPRECATED("Used get_reference<FrameForceTpl<Scalar> >()",
...
struct CostDataContactImpulseTpl : public CostDataAbstractTpl<_Scalar> {
  ...
  CostDataContactImpulseTpl(Model<Scalar> *const model,
        ...
        if (d3d != NULL) {
          ...
          if (model->get_activation()->get_nr() != 3) {
            throw_pretty("Domain error: nr isn't defined as 3 in the "
                         "activation model for the 3d impulse in " +
                         frame_name);
          }
          found_impulse = true;
          impulse = it->second;
