 ...
 * states, respetively, which belong to the state manifold \f$\mathcal{X}\f$.
...
public:
   ...
   *
   * We use `ActivationModelQuadTpl` as a default activation model (i.e.
   * \f$a=\frac{1}{2}\|\mathbf{r}\|^2\f$). The default reference state is
   * obtained from `StateAbstractTpl::zero()`.
   *
   * @param[in] state  State of the multibody system
   * @param[in] nu     Dimension of the control vector
   */
   ...
   *
   * We use `ActivationModelQuadTpl` as a default activation model (i.e.
   * \f$a=\frac{1}{2}\|\mathbf{r}\|^2\f$). The default state reference is
   * obtained from `StateAbstractTpl::zero()` and `nu` from
   * `StateAbstractTpl::get_nv()`.
   *
   * @param[in] state  State of the multibody system
   */
  explicit CostModelStateTpl(
  ...
  DEPRECATED("Use set_reference<MathBaseTpl<Scalar>::VectorXs>()",
             void set_xref(const VectorXs &xref_in));
  DEPRECATED("Use get_reference<MathBaseTpl<Scalar>::VectorXs>()",
...
struct CostDataStateTpl : public CostDataAbstractTpl<_Scalar> {
  ...
  CostDataStateTpl(Model<Scalar> *const model,
