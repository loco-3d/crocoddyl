 ...
 * \f$\mathbf{r}=\mathbf{p}\ominus\mathbf{p}^*\f$, where
 * \f$\mathbf{p},\mathbf{p}^*\in~\mathbb{SE(3)}\f$ are the current and reference
 * frame placements, respectively. Note that the dimension of the residual
 * vector is 6.
 *
...
public:
   ...
   *
   * We use `ActivationModelQuadTpl` as a default activation model (i.e.
   * \f$a=\frac{1}{2}\|\mathbf{r}\|^2\f$). Furthermore, the default `nu` is
  ...
  DEPRECATED("Use set_reference<FramePlacementTpl<Scalar> >()",
             void set_Mref(const FramePlacement &Mref_in));
  DEPRECATED("Use get_reference<FramePlacementTpl<Scalar> >()",
...
struct CostDataFramePlacementTpl : public CostDataAbstractTpl<_Scalar> {
  ...
  CostDataFramePlacementTpl(Model<Scalar> *const model,
                            DataCollectorAbstract *const data)
      : Base(model, data), J(6, model->get_state()->get_nv()), rJf(6, 6),
