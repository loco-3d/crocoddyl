...
public:
   ...
   * equals to StateAbstractTpl::get_nv()
   *
   * @param[in] state       State of the multibody system
   * @param[in] activation  Activation model
   * @param[in] Fref        Reference frame rotation
  ...
  DEPRECATED("Use set_reference<FrameRotationTpl<Scalar> >()",
             void set_Rref(const FrameRotation &Rref_in));
  DEPRECATED("Use get_reference<FrameRotationTpl<Scalar> >()",
...
struct CostDataFrameRotationTpl : public CostDataAbstractTpl<_Scalar> {
  ...
  CostDataFrameRotationTpl(Model<Scalar> *const model,
                           DataCollectorAbstract *const data)
      : Base(model, data), J(3, model->get_state()->get_nv()), rJf(3, 3),
