...
template <typename Scalar> CostModelStateTpl<Scalar>::~CostModelStateTpl() {}

template <typename Scalar>
void CostModelStateTpl<Scalar>::calc(
...
template <typename Scalar>
void CostModelStateTpl<Scalar>::calcDiff(
    const boost::shared_ptr<CostDataAbstract> &data,
    const Eigen::Ref<const VectorXs> &x, const Eigen::Ref<const VectorXs> &) {
  if (static_cast<std::size_t>(x.size()) != state_->get_nx()) {
    throw_pretty("Invalid argument: "
  ...
  if (pin_model_) {
    ...
    for (pinocchio::JointIndex i = 1;
      ...
      const MatrixBlock &RxBlock =
          data->Rx.block(pin_model_->idx_vs[i], pin_model_->idx_vs[i],
                         pin_model_->nvs[i], pin_model_->nvs[i]);
      data->Lx.segment(pin_model_->idx_vs[i], pin_model_->nvs[i]).noalias() =
          RxBlock.transpose() * data->activation->Ar.segment(
                                    pin_model_->idx_vs[i], pin_model_->nvs[i]);
      data->Lxx
          .block(pin_model_->idx_vs[i], pin_model_->idx_vs[i],
                 pin_model_->nvs[i], pin_model_->nvs[i])
          .noalias() = RxBlock.transpose() *
                       data->activation->Arr.diagonal()
                           .segment(pin_model_->idx_vs[i], pin_model_->nvs[i])
                           .asDiagonal() *
                       RxBlock;
    }
    data->Lx.tail(state_->get_nv()) =
        data->activation->Ar.tail(state_->get_nv());
    data->Lxx.diagonal().tail(state_->get_nv()) =
        data->activation->Arr.diagonal().tail(state_->get_nv());
  } else {
...
CostModelStateTpl<Scalar>::createData(DataCollectorAbstract *const data) {
  return boost::make_shared<CostDataStateTpl<Scalar>>(this, data);
}

template <typename Scalar>
void CostModelStateTpl<Scalar>::set_referenceImpl(const std::type_info &ti,
                                                  const void *pv) {
  if (ti == typeid(VectorXs)) {
    if (static_cast<std::size_t>(static_cast<const VectorXs *>(pv)->size()) !=
        state_->get_nx()) {
      throw_pretty("Invalid argument: "
                   << "reference has wrong dimension (it should be " +
                          std::to_string(state_->get_nx()) + ")");
    }
...
template <typename Scalar>
void CostModelStateTpl<Scalar>::get_referenceImpl(const std::type_info &ti,
                                                  void *pv) const {
  if (ti == typeid(VectorXs)) {
    VectorXs &tmp = *static_cast<VectorXs *>(pv);
    tmp.resize(state_->get_nx());
    Eigen::Map<VectorXs> ref_map(static_cast<VectorXs *>(pv)->data(),
                                 state_->get_nx());
...
CostModelStateTpl<Scalar>::get_xref() const {
  return xref_;
}
