 ...
 * reference spatial forces, respetively. The current spatial forces
 * \f$\boldsymbol{\lambda}\in\mathbb{R}^{nc}\f$is computed by
 * `DifferentialActionModelContactFwdDynamicsTpl`, with `nc` as the dimension of
 * the contact.
 *
 ...
 * computed by `DifferentialActionModelContactFwdDynamicsTpl`. These values are
 * stored in a shared data (i.e. DataCollectorContactTpl). Note that this cost
 * function cannot be used with other action models. For the computation of the
 * cost Hessian, we use the Gauss-Newton approximation, e.g. \f$\mathbf{l_{xu}}
...
public:
   ...
   *
   * Note that the `nr`, defined in the activation model, has to be lower /
   * equals than 6.
   *
   * @param[in] state       Multibody state
   * @param[in] activation  Activation model
   * @param[in] fref        Reference spatial contact force
   ...
   * the `nr`, defined in the activation model, has to be lower / equals than 6.
   *
   * @param[in] state       Multibody state
   * @param[in] activation  Activation model
   * @param[in] fref        Reference spatial contact force
   ...
   * \f$a=\frac{1}{2}\|\mathbf{r}\|^2\f$). Note that the `nr`, defined in the
   * activation model, has to be lower / equals than 6.
   *
   * @param[in] state  Multibody state
   * @param[in] fref   Reference spatial contact force
   * \f$\boldsymbol{\lambda}^*\f$
   ...
   *
   * We use `ActivationModelQuadTpl` as a default activation model (i.e.
   * \f$a=\frac{1}{2}\|\mathbf{r}\|^2\f$), and `nu` is obtained from
   * `StateAbstractTpl::get_nv()`. Note that the `nr`, defined in the activation
   ...
   *
   * We use `ActivationModelQuadTpl` as a default activation model (i.e.
   * \f$a=\frac{1}{2}\|\mathbf{r}\|^2\f$), and `nr` and `nu` are equals to 6 and
   * `StateAbstractTpl::get_nv()`, respectively.
   *
   * @param[in] state  Multibody state
   * @param[in] fref   Reference spatial contact force
   * \f$\boldsymbol{\lambda}^*\f$
   ...
   *
   * The force vector is computed by
   * DifferentialActionModelContactFwdDynamicsTpl and stored in
   ...
   *
   * The force derivatives are computed by
   * DifferentialActionModelContactFwdDynamicsTpl and stored in
   * DataCollectorContactTpl.
   *
   * @param[in] data  Contact force data
  ...
  DEPRECATED("Use set_reference<FrameForceTpl<Scalar> >()",
             void set_fref(const FrameForce &fref));
  DEPRECATED("Use get_reference<FrameForceTpl<Scalar> >()",
             const FrameForce &get_fref() const);

protected:
  /**
   * @brief Return the reference spatial contact force
   * \f$\boldsymbol{\lambda}^*\f$
   */
  virtual void set_referenceImpl(const std::type_info &ti, const void *pv);

  /**
   * @brief Modify the reference spatial contact force
   * \f$\boldsymbol{\lambda}^*\f$
   */
...
protected:
  FrameForce
      fref_; //!< Reference spatial contact force \f$\boldsymbol{\lambda}^*\f$
};
...
struct CostDataContactForceTpl : public CostDataAbstractTpl<_Scalar> {
  ...
  CostDataContactForceTpl(Model<Scalar> *const model,
      ...
      if (it->second->frame == fref.id) {
        ...
        if (d3d != NULL) {
          ...
          if (model->get_activation()->get_nr() != 3) {
            throw_pretty("Domain error: nr isn't defined as 3 in the "
                         "activation model for the 3d contact in " +
                         frame_name);
          }
          found_contact = true;
          contact = it->second;
        ...
        if (d6d != NULL) {
          ...
          if (model->get_activation()->get_nr() != 6) {
            throw_pretty("Domain error: nr isn't defined as 6 in the "
                         "activation model for the 3d contact in " +
                         frame_name);
          }
          found_contact = true;
