...
CostModelContactCoPPositionTpl<_Scalar>::CostModelContactCoPPositionTpl(boost::shared_ptr<StateMultibody> state,
                                          ...
                                          const std::size_t& frame_id, const Vector2s& cop_region, 
                                          const Vector3s& normal, const std::size_t& nu)
    : Base(state, activation, nu), frame_id_(frame_id), cop_region_(cop_region), normal_(normal) {
    }
...
void CostModelContactCoPPositionTpl<Scalar>::calc(const boost::shared_ptr<CostDataAbstract>& data,
  ...
  data->r.noalias() = d->A * d->f.toVector();
...
template<typename Scalar>
const std::size_t& CostModelContactCoPPositionTpl<Scalar>::get_frame_id() const {
  return frame_id_;
}
