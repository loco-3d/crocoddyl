...
void CostModelControlGravTpl<Scalar>::calc(const boost::shared_ptr<CostDataAbstract>& data,
  ...
  data->r = u - pinocchio::rnea(*pin_model_,*(d->pinocchio),x.head(state->get_nq()),x.tail(state->get_nv()),Eigen::VectorXd::Zero(state->get_nv()));
...
void CostModelControlGravTpl<Scalar>::calcDiff(const boost::shared_ptr<CostDataAbstract>& data,
  ...
  Data* d = static_cast<Data*>(data.get());
  const boost::shared_ptr<StateMultibody>& state = boost::static_pointer_cast<StateMultibody>(state_);
  pinocchio::computeRNEADerivatives(*pin_model_,*(d->pinocchio),x.head(state->get_nq()),x.tail(state->get_nv()),Eigen::VectorXd::Zero(state->get_nv()),
                                    d->rnea_partial_dx.topRows(state->get_nq()),d->rnea_partial_dx.bottomRows(state->get_nv()),d->rnea_partial_da);
