...
void CostModelControlGravTpl<Scalar>::calc(const boost::shared_ptr<CostDataAbstract>& data,
  ...
  data->r = u - pinocchio::rnea(*pin_model_,*(d->pinocchio),q,v,Eigen::VectorXd::Zero(state_->get_nv()));
...
void CostModelControlGravTpl<Scalar>::calcDiff(const boost::shared_ptr<CostDataAbstract>& data,
  ...
  pinocchio::computeRNEADerivatives(*pin_model_,*(d->pinocchio),q,v,Eigen::VectorXd::Zero(state_->get_nv()),
                                    d->dg_dx.topRows(state_->get_nq()),d->dg_dx.bottomRows(state_->get_nv()),d->dg_da);
