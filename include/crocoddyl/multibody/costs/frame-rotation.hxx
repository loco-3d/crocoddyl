...
CostModelFrameRotationTpl<Scalar>::CostModelFrameRotationTpl(
    ...
    : Base(state, activation, nu), Rref_(Rref),
      oRf_inv_(Rref.rotation.transpose()), pin_model_(state->get_pinocchio()) {
...
template <typename Scalar>
CostModelFrameRotationTpl<Scalar>::CostModelFrameRotationTpl(
    boost::shared_ptr<StateMultibody> state, const FrameRotation &Rref,
    const std::size_t &nu)
...
template <typename Scalar>
void CostModelFrameRotationTpl<Scalar>::calc(
...
template <typename Scalar>
void CostModelFrameRotationTpl<Scalar>::calcDiff(
    const boost::shared_ptr<CostDataAbstract> &data,
    const Eigen::Ref<const VectorXs> &, const Eigen::Ref<const VectorXs> &) {
  ...
  pinocchio::getFrameJacobian(*pin_model_.get(), *d->pinocchio, Rref_.id,
...
CostModelFrameRotationTpl<Scalar>::createData(
...
template <typename Scalar>
void CostModelFrameRotationTpl<Scalar>::set_referenceImpl(
  ...
  if (ti == typeid(FrameRotation)) {
    ...
    throw_pretty(
        "Invalid argument: incorrect type (it should be FrameRotation)");
  }
}

template <typename Scalar>
void CostModelFrameRotationTpl<Scalar>::get_referenceImpl(
...
template <typename Scalar>
const FrameRotationTpl<Scalar> &
CostModelFrameRotationTpl<Scalar>::get_Rref() const {
  return Rref_;
}
