 ...
 * its derivatives are computed by `ActionModelImpulseFwdDynamicsTpl`. These
 * values are stored in a shared data (i.e. `DataCollectorImpulseTpl`). Note
 * that this cost function cannot be used with other action models.
 *
...
public:
   ...
   * force vector is computed by `ActionModelImpulseFwdDynamicsTpl` which is
   ...
   * `ActionModelImpulseFwdDynamicsTpl` which are stored in
...
private:
  FrameCoPSupport cop_support_; //!< Frame name of the impulse foot and support
                                //!< region of the CoP
};
...
struct CostDataImpulseCoPPositionTpl : public CostDataAbstractTpl<_Scalar> {
  ...
  CostDataImpulseCoPPositionTpl(Model<Scalar> *const model,
    ...
    for (typename ImpulseModelMultiple::ImpulseDataContainer::iterator it =
      ...
      if (it->second->frame == cop_support.get_id()) {
        ...
        if (d3d != NULL) {
          throw_pretty("Domain error: a 6d impulse model is required in " +
