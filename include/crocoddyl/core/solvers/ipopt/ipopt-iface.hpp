 ...
 *
 * This class implements the pure virtual functions from Ipopt::TNLP to solve the optimal control problem
 * in `problem_` using a multiple shooting approach.
 *
 * Ipopt considers its decision variables `x` to belong to the Euclidean space. However, Crocoddyl states could lie in
 * a manifold. To ensure that the solution of Ipopt lies in the manifold of the state, we perform the optimization in
 * the tangent space of a given initial state. Finally we retract the Ipopt solution to the manifold. That is:
 *  * \f[
 * \begin{aligned}
 ...
 * \f]
 *
 * where \f$\mathbf{x}^*\f$ is the final solution, \f$\mathbf{x}^0\f$ is the initial guess and \f$\mathbf{\Delta
 * x}^*\f$ is the Ipopt solution in the tangent space of \f$\mathbf{x}_0\f$.
 * Due to this procedure, the computation of the cost function, the dynamic constraint as well as their corresponding
 * derivatives should be properly modified.
 *
 * The Ipopt decision vector is built as follows: \f$x = [ \mathbf{\Delta x}_0^\top, \mathbf{u}_0^\top, \mathbf{\Delta
 * x}_1^\top, \mathbf{u}_1^\top, \dots, \mathbf{\Delta x}_N^\top ]\f$
 *
 * Dynamic constraints are posed as: \f$(\mathbf{x}^0_{k+1} \oplus \mathbf{\Delta x}_{k+1}) \ominus
 * \mathbf{f}(\mathbf{x}_{k}^0 \oplus \mathbf{\Delta x}_{k}, \mathbf{u}_k) = \mathbf{0}\f$
 *
 * Initial condition: \f$ \mathbf{x}(0) \ominus (\mathbf{x}_{k}^0 \oplus \mathbf{\Delta x}_{k}) = \mathbf{0}\f$
 *
 * Documentation of the methods has been extracted from Ipopt::TNLP.hpp file
 *
 *  \sa `get_nlp_info()`, `get_bounds_info()`, `eval_f()`, `eval_g()`, `eval_grad_f()`, `eval_jac_g()`, `eval_h()`
 */
