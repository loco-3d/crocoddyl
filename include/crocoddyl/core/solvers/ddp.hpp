 ...
 * \f$\mathbf{\bar{f}}_s\f$ and run a modified Riccati sweep: \f{eqnarray*}
...
public:
   ...
   * have been compute (i.e. `calcDiff()`). The backward pass handles infeasible
   * guess through a modified Riccati sweep: \f{eqnarray*}
   *   \mathbf{Q}_{\mathbf{x}_k} &=& \mathbf{l}_{\mathbf{x}_k} +
   * \mathbf{f}^\top_{\mathbf{x}_k} (V_{\mathbf{x}_{k+1}}
   * +
   * V_{\mathbf{xx}_{k+1}}\mathbf{\bar{f}}_{k+1}),\\
