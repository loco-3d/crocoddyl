 ...
 *
 * It applies a standard RK integration schemes to a differential (i.e., continuous time) action model.
 * The available integrators are: RK2, RK3, and RK4.
 *
 * This standard RK scheme introduces also the possibility to parametrize the control trajectory inside an
...
class IntegratedActionModelRKTpl : public IntegratedActionModelAbstractTpl<_Scalar> {
 public:
  ...
  IntegratedActionModelRKTpl(boost::shared_ptr<DifferentialActionModelAbstract> model,
                             boost::shared_ptr<ControlParametrizationModelAbstract> control, const RKType rktype,
   ...
   * @param[in] with_cost_residual  Compute cost residual (default true)
   */
  IntegratedActionModelRKTpl(boost::shared_ptr<DifferentialActionModelAbstract> model, const RKType rktype,
                             const Scalar time_step = Scalar(1e-3), const bool with_cost_residual = true);
  ...
  /**
   * @brief Integrate the total cost value for nodes that depends only on the state using RK scheme
   *
...
template <typename _Scalar>
struct IntegratedActionDataRKTpl : public IntegratedActionDataAbstractTpl<_Scalar> {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  std::vector<VectorXs> y;   //!< List of states where f is evaluated in the RK integration
  ...
  std::vector<MatrixXs>
      dyi_dx;  //!< List of partial derivatives of RK dynamics with respect to the state of the RK integrator. dyi/dx
  std::vector<MatrixXs> dyi_du;  //!< List of partial derivatives of RK dynamics with respect to the control
                                 //!< parameters of the RK integrator. dyi/du

  std::vector<VectorXs>
      dli_dx;  //!< List of partial derivatives of the cost with respect to the state of the RK integration. dli_dx
  std::vector<VectorXs> dli_du;  //!< List of partial derivatives of the cost with respect to the control input of the
                                 //!< RK integration. dli_du

  std::vector<MatrixXs> ddli_ddx;  //!< List of second partial derivatives of the cost with respect to the state of the
                                   //!< RK integration. ddli_ddx
  std::vector<MatrixXs> ddli_ddw;  //!< List of second partial derivatives of the cost with respect to the control
                                   //!< parameters of the RK integration. ddli_ddw
