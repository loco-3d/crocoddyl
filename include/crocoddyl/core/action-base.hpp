 ...
 *
 * In Crocoddyl, an action model combines dynamics and cost models. Each node,
 ...
 * the dynamics, cost functions and their derivatives. All these is described
 * inside the action model.
 *
 * Concretely speaking, the action model describes a time-discrete action model
 * with a first-order ODE along a cost function, i.e. \f[ \begin{aligned}
 * &\delta\mathbf{x}^+ =
 * \mathbf{f_{x}}\delta\mathbf{x}+\mathbf{f_{u}}\delta\mathbf{u},
 * &\textrm{(dynamics)}\\ &l(\delta\mathbf{x},\delta\mathbf{u}) =
 * \begin{bmatrix}1 \\ \delta\mathbf{x} \\ \delta\mathbf{u}\end{bmatrix}^T
 * \begin{bmatrix}0 & \mathbf{l_x}^T & \mathbf{l_u}^T \\ \mathbf{l_x} &
 * \mathbf{l_{xx}} & \mathbf{l_{ux}}^T \\
 * \mathbf{l_u} & \mathbf{l_{ux}} & \mathbf{l_{uu}}\end{bmatrix}
 * \begin{bmatrix}1 \\ \delta\mathbf{x} \\ \delta\mathbf{u}\end{bmatrix},
 * &\textrm{(cost)} \end{aligned} \f] where the state
 * \f$\mathbf{x}\in\mathcal{X}\f$ lies in the state manifold described with a
 * `nx`-tuple, its rate \f$\delta\mathbf{x}\in T_{\mathbf{x}}\mathcal{X}\f$ is a
 * tangent vector to this manifold with `ndx` dimension, and
 * \f$\mathbf{u}\in\mathbb{R}^{nu}\f$ is the input commands. Note that the we
 * could describe a linear or linearized action system, where the cost has a
 * quadratic form.
 *
 * The main computations are carrying out in `calc` and `calcDiff`. `calc`
 * computes the next state and cost and `calcDiff` computes the derivatives of
 * the dynamics and cost function. Concretely speaking, `calcDiff` builds a
 * linear-quadratic approximation of an action model, where the dynamics and
 * cost functions have linear and quadratic forms, respectively.
 * \f$\mathbf{f_x}\in\mathbb{R}^{nv\times ndx}\f$,
 * \f$\mathbf{f_u}\in\mathbb{R}^{nv\times nu}\f$ are the Jacobians of the
 * dynamics; \f$\mathbf{l_x}\in\mathbb{R}^{ndx}\f$,
 ...
 * Hessians of the cost function, respectively. Additionally, it is important
 * remark that `calcDiff()` computes the derivates using the latest stored
 * values by `calc()`. Thus, we need to run first `calc()`.
 *
 * \sa `calc()`, `calcDiff()`, `createData()`
 */
template <typename _Scalar> class ActionModelAbstractTpl {
public:
...
template <typename _Scalar> struct ActionDataAbstractTpl {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  explicit ActionDataAbstractTpl(Model<Scalar> *const model)
      : cost(Scalar(0.)), xnext(model->get_state()->get_nx()),
        Fx(model->get_state()->get_ndx(), model->get_state()->get_ndx()),
        Fu(model->get_state()->get_ndx(), model->get_nu()), r(model->get_nr()),
        Lx(model->get_state()->get_ndx()), Lu(model->get_nu()),
