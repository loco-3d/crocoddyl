...
template <typename _Scalar> struct CostItemTpl {
 ...
 * `calcDiff` builds a linear-quadratic approximation of the total cost function
 ...
 */
template <typename _Scalar> class CostModelSumTpl {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
...
template <typename _Scalar> struct CostDataSumTpl {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  CostDataSumTpl(Model<Scalar> *const model, DataCollectorAbstract *const data)
      : Lx_internal(model->get_state()->get_ndx()),
        ...
        Luu_internal(model->get_nu(), model->get_nu()), shared(data),
        cost(Scalar(0.)), Lx(Lx_internal.data(), model->get_state()->get_ndx()),
  ...
  template <class ActionData> void shareMemory(ActionData *const data) {
