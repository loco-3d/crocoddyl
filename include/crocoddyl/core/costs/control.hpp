 ...
 * reference control inputs, respetively. Note that the dimension of the
...
public:
   ...
   * `MathBaseTpl<>::VectorXs::Zero(nu)` with `nu` obtained by
   ...
   * We use `ActivationModelQuadTpl` as a default activation model (i.e.
   * \f$a=\frac{1}{2}\|\mathbf{r}\|^2\f$). The default reference control is
   * obtained from `MathBaseTpl<>::VectorXs::Zero(nu)`.
   *
   * @param[in] state       State of the multibody system
   * @param[in] nu          Dimension of the control vector
   */
  ...
  DEPRECATED("Use set_reference<MathbTpl<Scalare>::VectorXs>()",
             void set_uref(const VectorXs &uref_in));
  DEPRECATED("Use get_reference<MathbTpl<Scalare>::VectorXs>()",
