...
template <typename Scalar> CostModelSumTpl<Scalar>::~CostModelSumTpl() {}
...
void CostModelSumTpl<Scalar>::addCost(const std::string &name,
  ...
  if (ret.second == false) {
    ...
    std::vector<std::string>::iterator it = std::lower_bound(
        active_.begin(), active_.end(), name, std::less<std::string>());
    active_.insert(it, name);
  } else if (!active) {
    nr_total_ += cost->get_activation()->get_nr();
    std::vector<std::string>::iterator it = std::lower_bound(
        inactive_.begin(), inactive_.end(), name, std::less<std::string>());
...
void CostModelSumTpl<Scalar>::removeCost(const std::string &name) {
  ...
  if (it != costs_.end()) {
    ...
    costs_.erase(it);
    active_.erase(std::remove(active_.begin(), active_.end(), name),
                  active_.end());
    inactive_.erase(std::remove(inactive_.begin(), inactive_.end(), name),
                    inactive_.end());
  } else {
...
template <typename Scalar>
boost::shared_ptr<CostDataSumTpl<Scalar>>
CostModelSumTpl<Scalar>::createData(DataCollectorAbstract *const data) {
  return boost::allocate_shared<CostDataSum>(
      Eigen::aligned_allocator<CostDataSum>(), this, data);
}

template <typename Scalar>
void CostModelSumTpl<Scalar>::calc(
    const boost::shared_ptr<CostDataSumTpl<Scalar>> &data,
    const Eigen::Ref<const VectorXs> &x) {
  calc(data, x, unone_);
}
