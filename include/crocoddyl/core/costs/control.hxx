...
CostModelControlTpl<Scalar>::CostModelControlTpl(
    ...
    : Base(state, activation, static_cast<std::size_t>(uref.size())),
      uref_(uref) {
...
CostModelControlTpl<Scalar>::CostModelControlTpl(
    ...
    : Base(state, static_cast<std::size_t>(uref.size()),
           static_cast<std::size_t>(uref.size())),
      uref_(uref) {}
...
template <typename Scalar>
void CostModelControlTpl<Scalar>::calc(
...
template <typename Scalar>
void CostModelControlTpl<Scalar>::calcDiff(
    const boost::shared_ptr<CostDataAbstract> &data,
    const Eigen::Ref<const VectorXs> &, const Eigen::Ref<const VectorXs> &u) {
  if (nu_ == 0) {
    throw_pretty("Invalid argument: "
...
template <typename Scalar>
void CostModelControlTpl<Scalar>::set_referenceImpl(const std::type_info &ti,
                                                    const void *pv) {
...
template <typename Scalar>
void CostModelControlTpl<Scalar>::get_referenceImpl(const std::type_info &ti,
                                                    void *pv) const {
  if (ti == typeid(VectorXs)) {
...
CostModelControlTpl<Scalar>::get_uref() const {
  return uref_;
}
...
void CostModelControlTpl<Scalar>::set_uref(const VectorXs &uref_in) {
  if (static_cast<std::size_t>(uref_in.size()) != nu_) {
    throw_pretty("Invalid argument: "
                 << "uref has wrong dimension (it should be " +
                        std::to_string(nu_) + ")");
  }
  uref_ = uref_in;
}
