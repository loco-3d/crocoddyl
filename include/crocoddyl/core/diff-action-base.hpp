 ...
 * &\textrm{(dynamics)}\\ &l(\mathbf{x},\mathbf{u}) = \int_0^{\delta t}
 * a(\mathbf{r}(\mathbf{x},\mathbf{u}))\,dt, &\textrm{(cost)} \end{aligned} \f]
 * where the configuration \f$\mathbf{q}\in\mathcal{Q}\f$ lies in the
 * configuration manifold described with a `nq`-tuple, the velocity
 * \f$\mathbf{v}\in T_{\mathbf{q}}\mathcal{Q}\f$ its a tangent vector to this
 * manifold with `nv` dimension, \f$\mathbf{u}\in\mathbb{R}^{nu}\f$ is the input
 * commands, \f$\mathbf{r}\f$ and \f$a\f$ is a residual and activation functions
 * (see `ActivationModelAbstractTpl`). Both configuration and velocity describe
 * the system space \f$\mathbf{x}\in\mathbf{X}\f$ which lies in the state
 * manifold. Note that the acceleration \f$\dot{\mathbf{v}}\in
 ...
 * computes the acceleration and cost and `calcDiff` computes the derivatives of
 * the dynamics and cost function. Concretely speaking, `calcDiff` builds a
 * linear-quadratic approximation of the differential action, where the dynamics
 * and cost functions have linear and quadratic forms, respectively.
 ...
 */
template <typename _Scalar> class DifferentialActionModelAbstractTpl {
public:
   ...
   */
  VectorXs
  quasiStatic_x(const boost::shared_ptr<DifferentialActionDataAbstract> &data,
...
template <typename _Scalar> struct DifferentialActionDataAbstractTpl {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  explicit DifferentialActionDataAbstractTpl(Model<Scalar> *const model)
      : cost(0.), xout(model->get_state()->get_nv()),
        Fx(model->get_state()->get_nv(), model->get_state()->get_ndx()),
        Fu(model->get_state()->get_nv(), model->get_nu()), r(model->get_nr()),
