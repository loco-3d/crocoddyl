...
class SquashingModelSmoothSatTpl : public SquashingModelAbstractTpl<_Scalar> {
 public:
  ...
  explicit SquashingModelSmoothSatTpl(const Eigen::Ref<const VectorXs>& out_lb,
                                      const Eigen::Ref<const VectorXs>& out_ub, const std::size_t& ns)
      : Base(ns) {
    out_lb_ = out_lb;
    out_ub_ = out_ub;
  ...
  void calc(const boost::shared_ptr<SquashingDataAbstract>& data, const Eigen::Ref<const VectorXs>& u) {
    ...
    data->s =
        0.5 * (Eigen::sqrt(Eigen::pow((u - out_lb_).array(), 2) + a_.array()) -
               Eigen::sqrt(Eigen::pow((u - out_ub_).array(), 2) + a_.array()) + out_lb_.array() + out_ub_.array());
  }
  ...
  void calcDiff(const boost::shared_ptr<SquashingDataAbstract>& data, const Eigen::Ref<const VectorXs>& u,
    ...
    data->ds_du.diagonal() =
        0.5 * (Eigen::pow(a_.array() + Eigen::pow((u - out_lb_).array(), 2), -0.5).array() * (u - out_lb_).array() -
               Eigen::pow(a_.array() + Eigen::pow((u - out_ub_).array(), 2), -0.5).array() * (u - out_ub_).array());
  }
