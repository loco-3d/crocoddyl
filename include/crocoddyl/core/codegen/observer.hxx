///////////////////////////////////////////////////////////////////////////////
// BSD 3-Clause License
//
// Copyright (C) 2019-2026, LAAS-CNRS, INRIA, University of Edinburgh,
//                          Heriot-Watt University
// Copyright note valid unless otherwise stated in individual files.
// All rights reserved.
///////////////////////////////////////////////////////////////////////////////

#ifndef CROCODDYL_CORE_CODEGEN_OBSERVER_HXX_
#define CROCODDYL_CORE_CODEGEN_OBSERVER_HXX_

#ifdef CROCODDYL_WITH_CODEGEN

namespace crocoddyl {

template <typename Scalar>
ObserverModelCodeGenTpl<Scalar>::ObserverModelCodeGenTpl(
    std::shared_ptr<Base> model, const std::string& lib_fname, bool autodiff,
    const std::size_t np, ParamsEnvironment updateParams, CompilerType compiler,
    const std::string& compile_options)
    : ObserverModelCodeGenTpl(model, lib_fname, autodiff, np, 0u, updateParams,
                              compiler, compile_options) {}

template <typename Scalar>
ObserverModelCodeGenTpl<Scalar>::ObserverModelCodeGenTpl(
    std::shared_ptr<Base> model, const std::string& lib_fname, bool autodiff,
    const std::size_t np, const std::size_t nenv,
    ParamsEnvironment updateParams, CompilerType compiler,
    const std::string& compile_options,
    const std::string& state_observation_cost, const std::string& weight_cost)
    : Base(model->get_state(), model->get_ntau(), model->get_nu(),
           model->get_nr(), model->get_ng(), model->get_nh(), model->get_ng_T(),
           model->get_nh_T(), np == 0u ? model->get_np() : np),
      model_(model),
      ad_model_(
          std::dynamic_pointer_cast<ADBase>(model->template cast<ADScalar>())),
      ad_data_(nullptr),
      ad_data_pert_(nullptr),
      autodiff_(autodiff),
      np_(np == 0u ? model->get_np() : np),
      nenv_(nenv),
      nX_(state_->get_nx() + nu_ + np_ + nenv_ + ntau_),
      nX_T_(state_->get_nx() + np_ + nenv_ + ntau_),
      nX2_(nX_ + (autodiff ? state_->get_ndx() + nu_ + np_ : 0u)),
      nX2_T_(nX_T_ + (autodiff ? state_->get_ndx() + np_ : 0u)),
      nY1_(1 + state_->get_nx() + ng_ + nh_ + 1),
      nY1_T_(1 + ng_T_ + nh_T_ + 1),
      ad_X_(nX_),
      ad_X_T_(nX_T_),
      ad_X2_(nX2_),
      ad_X2_T_(nX2_T_),
      ad_Y1_(nY1_),
      ad_Y1_T_(nY1_T_),
      Y1fun_name_("observer_calc"),
      Y1Tfun_name_("observer_calc_T"),
      Y2fun_name_("observer_calcDiff"),
      Y2Tfun_name_("observer_calcDiff_T"),
      Y2Costfun_name_("observer_calcDiff_cost"),
      Y2CostTfun_name_("observer_calcDiff_cost_T"),
      lib_fname_(lib_fname),
      compiler_type_(compiler),
      compile_options_(compile_options),
      updateParams_(updateParams),
      ad_calc_(std::make_unique<ADFun>()),
      ad_calc_T_(std::make_unique<ADFun>()),
      ad_calcDiff_(std::make_unique<ADFun>()),
      ad_calcDiff_T_(std::make_unique<ADFun>()),
      ad_calcDiffCost_(std::make_unique<ADFun>()),
      ad_calcDiffCost_T_(std::make_unique<ADFun>()) {
  (void)state_observation_cost;
  (void)weight_cost;
  if (!ad_model_) {
    throw_pretty(
        "Invalid argument: observer model cannot be cast to an AD observer");
  }
  const std::size_t ndx = state_->get_ndx();
  const std::size_t full_Y2 =
      ndx * ndx + ndx * nu_ + ndx * np_ + ndx + nu_ + np_ + ndx * ndx +
      ndx * nu_ + nu_ * nu_ + np_ * np_ + np_ * ndx + np_ * nu_ +
      ng_ * (ndx + nu_ + np_) + nh_ * (ndx + nu_ + np_) + ndx + nu_ + np_;
  const std::size_t full_Y2_T = ndx + np_ + ndx * ndx + np_ * np_ + np_ * ndx +
                                (ng_T_ + nh_T_) * (ndx + np_) + ndx + np_;
  if (autodiff_) {
    nY2_ = 1 + ndx + ng_ + nh_ + 1;
    nY2_T_ = 1 + ndx + ng_T_ + nh_T_ + 1;
    nYh_ = full_Y2;
    nYh_T_ = full_Y2_T;
  } else {
    nY2_ = full_Y2;
    nY2_T_ = full_Y2_T;
    nYh_ = ndx * ndx + ndx * nu_ + nu_ * nu_;
    nYh_ += np_ * np_ + np_ * ndx + np_ * nu_;
    nYh_T_ = ndx * ndx + np_ * np_ + np_ * ndx;
  }
  ad_Y2_.resize(nY2_);
  ad_Y2_T_.resize(nY2_T_);
  this->set_g_lb(model->get_g_lb());
  this->set_g_ub(model->get_g_ub());
  setupADParameters();
  initLib();
  compileLib();
  loadLib(lib_fname_);
  wCostHess_ = VectorXs::Zero(nY2_);
  wCostHess_(0) = Scalar(1.);
  wCostHess_T_ = VectorXs::Zero(nY2_T_);
  wCostHess_T_(0) = Scalar(1.);
  wCostHessScalar_ = VectorXs::Ones(1);
  wCostHessScalar_T_ = VectorXs::Ones(1);
}

template <typename Scalar>
ObserverModelCodeGenTpl<Scalar>::ObserverModelCodeGenTpl(
    std::shared_ptr<Base> model, const std::string& lib_fname, bool autodiff,
    const std::size_t np, const std::size_t nenv,
    const std::string& state_observation_cost, const std::string& weight_cost,
    CompilerType compiler, const std::string& compile_options)
    : ObserverModelCodeGenTpl(
          model, lib_fname, autodiff, np, nenv,
          makeStateObservationEnvironment(state_observation_cost, weight_cost),
          compiler, compile_options, state_observation_cost, weight_cost) {}

template <typename Scalar>
ObserverModelCodeGenTpl<Scalar>::ObserverModelCodeGenTpl(
    std::shared_ptr<ADBase> ad_model, const std::string& lib_fname,
    bool autodiff, const std::size_t np, ParamsEnvironment updateParams,
    CompilerType compiler, const std::string& compile_options)
    : ObserverModelCodeGenTpl(ad_model, lib_fname, autodiff, np, 0u,
                              updateParams, compiler, compile_options) {}

template <typename Scalar>
ObserverModelCodeGenTpl<Scalar>::ObserverModelCodeGenTpl(
    std::shared_ptr<ADBase> ad_model, const std::string& lib_fname,
    bool autodiff, const std::size_t np, const std::size_t nenv,
    ParamsEnvironment updateParams, CompilerType compiler,
    const std::string& compile_options,
    const std::string& state_observation_cost, const std::string& weight_cost)
    : Base(ad_model->get_state()->template cast<Scalar>(), ad_model->get_ntau(),
           ad_model->get_nu(), ad_model->get_nr(), ad_model->get_ng(),
           ad_model->get_nh(), ad_model->get_ng_T(), ad_model->get_nh_T(),
           np == 0u ? ad_model->get_np() : np),
      model_(
          std::dynamic_pointer_cast<Base>(ad_model->template cast<Scalar>())),
      ad_model_(ad_model),
      ad_data_(nullptr),
      ad_data_pert_(nullptr),
      autodiff_(autodiff),
      np_(np == 0u ? ad_model->get_np() : np),
      nenv_(nenv),
      nX_(state_->get_nx() + nu_ + np_ + nenv_ + ntau_),
      nX_T_(state_->get_nx() + np_ + nenv_ + ntau_),
      nX2_(nX_ + (autodiff ? state_->get_ndx() + nu_ + np_ : 0u)),
      nX2_T_(nX_T_ + (autodiff ? state_->get_ndx() + np_ : 0u)),
      nY1_(1 + state_->get_nx() + ng_ + nh_ + 1),
      nY1_T_(1 + ng_T_ + nh_T_ + 1),
      ad_X_(nX_),
      ad_X_T_(nX_T_),
      ad_X2_(nX2_),
      ad_X2_T_(nX2_T_),
      ad_Y1_(nY1_),
      ad_Y1_T_(nY1_T_),
      Y1fun_name_("observer_calc"),
      Y1Tfun_name_("observer_calc_T"),
      Y2fun_name_("observer_calcDiff"),
      Y2Tfun_name_("observer_calcDiff_T"),
      Y2Costfun_name_("observer_calcDiff_cost"),
      Y2CostTfun_name_("observer_calcDiff_cost_T"),
      lib_fname_(lib_fname),
      compiler_type_(compiler),
      compile_options_(compile_options),
      updateParams_(updateParams),
      ad_calc_(std::make_unique<ADFun>()),
      ad_calc_T_(std::make_unique<ADFun>()),
      ad_calcDiff_(std::make_unique<ADFun>()),
      ad_calcDiff_T_(std::make_unique<ADFun>()),
      ad_calcDiffCost_(std::make_unique<ADFun>()),
      ad_calcDiffCost_T_(std::make_unique<ADFun>()) {
  (void)state_observation_cost;
  (void)weight_cost;
  const std::size_t ndx = state_->get_ndx();
  const std::size_t full_Y2 =
      ndx * ndx + ndx * nu_ + ndx * np_ + ndx + nu_ + np_ + ndx * ndx +
      ndx * nu_ + nu_ * nu_ + np_ * np_ + np_ * ndx + np_ * nu_ +
      ng_ * (ndx + nu_ + np_) + nh_ * (ndx + nu_ + np_) + ndx + nu_ + np_;
  const std::size_t full_Y2_T = ndx + np_ + ndx * ndx + np_ * np_ + np_ * ndx +
                                (ng_T_ + nh_T_) * (ndx + np_) + ndx + np_;
  if (autodiff_) {
    nY2_ = 1 + ndx + ng_ + nh_ + 1;
    nY2_T_ = 1 + ndx + ng_T_ + nh_T_ + 1;
    nYh_ = full_Y2;
    nYh_T_ = full_Y2_T;
  } else {
    nY2_ = full_Y2;
    nY2_T_ = full_Y2_T;
    nYh_ = ndx * ndx + ndx * nu_ + nu_ * nu_;
    nYh_ += np_ * np_ + np_ * ndx + np_ * nu_;
    nYh_T_ = ndx * ndx + np_ * np_ + np_ * ndx;
  }
  ad_Y2_.resize(nY2_);
  ad_Y2_T_.resize(nY2_T_);
  setupADParameters();
  initLib();
  compileLib();
  loadLib(lib_fname_);
  wCostHess_ = VectorXs::Zero(nY2_);
  wCostHess_(0) = Scalar(1.);
  wCostHess_T_ = VectorXs::Zero(nY2_T_);
  wCostHess_T_(0) = Scalar(1.);
  wCostHessScalar_ = VectorXs::Ones(1);
  wCostHessScalar_T_ = VectorXs::Ones(1);
}

template <typename Scalar>
ObserverModelCodeGenTpl<Scalar>::ObserverModelCodeGenTpl(
    std::shared_ptr<ADBase> ad_model, const std::string& lib_fname,
    bool autodiff, const std::size_t np, const std::size_t nenv,
    const std::string& state_observation_cost, const std::string& weight_cost,
    CompilerType compiler, const std::string& compile_options)
    : ObserverModelCodeGenTpl(
          ad_model, lib_fname, autodiff, np, nenv,
          makeStateObservationEnvironment(state_observation_cost, weight_cost),
          compiler, compile_options, state_observation_cost, weight_cost) {}

template <typename Scalar>
ObserverModelCodeGenTpl<Scalar>::ObserverModelCodeGenTpl(
    const ObserverModelCodeGenTpl<Scalar>& other)
    : Base(other),
      model_(other.model_),
      ad_model_(other.ad_model_),
      ad_data_(nullptr),
      ad_data_pert_(nullptr),
      autodiff_(other.autodiff_),
      np_(other.np_),
      nenv_(other.nenv_),
      nX_(other.nX_),
      nX_T_(other.nX_T_),
      nX2_(other.nX2_),
      nX2_T_(other.nX2_T_),
      nY1_(other.nY1_),
      nY1_T_(other.nY1_T_),
      nY2_(other.nY2_),
      nY2_T_(other.nY2_T_),
      nYh_(other.nYh_),
      nYh_T_(other.nYh_T_),
      ad_X_(other.nX_),
      ad_X_T_(other.nX_T_),
      ad_X2_(other.nX2_),
      ad_X2_T_(other.nX2_T_),
      ad_Y1_(other.nY1_),
      ad_Y1_T_(other.nY1_T_),
      ad_Y2_(other.nY2_),
      ad_Y2_T_(other.nY2_T_),
      Y1fun_name_(other.Y1fun_name_),
      Y1Tfun_name_(other.Y1Tfun_name_),
      Y2fun_name_(other.Y2fun_name_),
      Y2Tfun_name_(other.Y2Tfun_name_),
      Y2Costfun_name_(other.Y2Costfun_name_),
      Y2CostTfun_name_(other.Y2CostTfun_name_),
      lib_fname_(other.lib_fname_),
      compiler_type_(other.compiler_type_),
      compile_options_(other.compile_options_),
      updateParams_(other.updateParams_),
      params_(other.params_),
      ad_calc_(clone_adfun(*other.ad_calc_)),
      ad_calc_T_(clone_adfun(*other.ad_calc_T_)),
      ad_calcDiff_(clone_adfun(*other.ad_calcDiff_)),
      ad_calcDiff_T_(clone_adfun(*other.ad_calcDiff_T_)),
      ad_calcDiffCost_(clone_adfun(*other.ad_calcDiffCost_)),
      ad_calcDiffCost_T_(clone_adfun(*other.ad_calcDiffCost_T_)),
      calcCG_(std::make_unique<CSourceGen>(*ad_calc_, Y1fun_name_)),
      calcCG_T_(std::make_unique<CSourceGen>(*ad_calc_T_, Y1Tfun_name_)),
      calcDiffCG_(std::make_unique<CSourceGen>(*ad_calcDiff_, Y2fun_name_)),
      calcDiffCG_T_(
          std::make_unique<CSourceGen>(*ad_calcDiff_T_, Y2Tfun_name_)),
      calcDiffCostCG_(
          std::make_unique<CSourceGen>(*ad_calcDiffCost_, Y2Costfun_name_)),
      calcDiffCostCG_T_(
          std::make_unique<CSourceGen>(*ad_calcDiffCost_T_, Y2CostTfun_name_)),
      libCG_(std::make_unique<LibraryCSourceGen>(
          *calcCG_, *calcCG_T_, *calcDiffCG_, *calcDiffCG_T_, *calcDiffCostCG_,
          *calcDiffCostCG_T_)),
      dynLibManager_(
          std::make_unique<LibraryProcessor>(*other.libCG_, lib_fname_)) {
  setupADParameters();
  loadLib(lib_fname_);
  wCostHess_ = other.wCostHess_;
  wCostHess_T_ = other.wCostHess_T_;
  wCostHessScalar_ = other.wCostHessScalar_;
  wCostHessScalar_T_ = other.wCostHessScalar_T_;
}

template <typename Scalar>
ObserverModelCodeGenTpl<Scalar>::ObserverModelCodeGenTpl(
    const std::string& lib_fname, std::shared_ptr<Base> model)
    : Base(model->get_state(), model->get_ntau(), model->get_nu(),
           model->get_nr(), model->get_ng(), model->get_nh(), model->get_ng_T(),
           model->get_nh_T(), model->get_np()),
      model_(model),
      Y1fun_name_("observer_calc"),
      Y1Tfun_name_("observer_calc_T"),
      Y2fun_name_("observer_calcDiff"),
      Y2Tfun_name_("observer_calcDiff_T"),
      Y2Costfun_name_("observer_calcDiff_cost"),
      Y2CostTfun_name_("observer_calcDiff_cost_T"),
      lib_fname_(lib_fname),
      ad_calc_(std::make_unique<ADFun>()),
      ad_calc_T_(std::make_unique<ADFun>()),
      ad_calcDiff_(std::make_unique<ADFun>()),
      ad_calcDiff_T_(std::make_unique<ADFun>()),
      ad_calcDiffCost_(std::make_unique<ADFun>()),
      ad_calcDiffCost_T_(std::make_unique<ADFun>()) {
  loadLib(lib_fname_);
  nX_ = calcFun_->Domain();
  nX_T_ = calcFun_T_->Domain();
  nY1_ = calcFun_->Range();
  nY1_T_ = calcFun_T_->Range();
  nX2_ = calcDiffFun_->Domain();
  nX2_T_ = calcDiffFun_T_->Domain();
  nY2_ = calcDiffFun_->Range();
  nY2_T_ = calcDiffFun_T_->Range();
  nYh_ = calcDiffCostFun_->Range();
  nYh_T_ = calcDiffCostFun_T_->Range();
  np_ = model->get_np();
  nenv_ = nX_T_ - state_->get_nx() - np_ - ntau_;
  const std::size_t nx = state_->get_nx();
  if (nX_ != nx + nu_ + np_ + nenv_ + ntau_) {
    throw_pretty(
        "The number of independent variables nX in the code generated library "
        "is not equal to the number of independent variables in the model");
  }
  if (nX_T_ != nx + np_ + nenv_ + ntau_) {
    throw_pretty(
        "The number of independent variables nX_T in the code generated "
        "library is not equal to the number of independent variables in the "
        "model");
  }
  if (nY1_ != 1 + nx + ng_ + nh_ + 1) {
    throw_pretty(
        "The number of dependent variables nY1 in the code generated library "
        "is not equal to the number of dependent variables in the model");
  }
  if (nY1_T_ != 1 + ng_T_ + nh_T_ + 1) {
    throw_pretty(
        "The number of dependent variables nY1_T in the code generated library "
        "is not equal to the number of dependent variables in the model");
  }
  ad_X_.resize(nX_);
  ad_X_T_.resize(nX_T_);
  ad_X2_.resize(nX2_);
  ad_X2_T_.resize(nX2_T_);
  ad_Y1_.resize(nY1_);
  ad_Y1_T_.resize(nY1_T_);
  ad_Y2_.resize(nY2_);
  ad_Y2_T_.resize(nY2_T_);
  autodiff_ = nX2_ != nX_;
  if (autodiff_ && (!calcDiffFun_->isJacobianAvailable() || !calcDiffCostFun_ ||
                    !calcDiffCostFun_T_)) {
    throw_pretty(
        "The code generated library contains a stale autodiff observer "
        "calcDiff tape; please regenerate it");
  }
  wCostHess_ = VectorXs::Zero(nY2_);
  wCostHess_(0) = Scalar(1.);
  wCostHess_T_ = VectorXs::Zero(nY2_T_);
  wCostHess_T_(0) = Scalar(1.);
  wCostHessScalar_ = VectorXs::Ones(1);
  wCostHessScalar_T_ = VectorXs::Ones(1);
}

template <typename Scalar>
ObserverModelCodeGenTpl<Scalar>::ObserverModelCodeGenTpl(
    const std::string& lib_fname, std::shared_ptr<ADBase> ad_model)
    : Base(ad_model->get_state()->template cast<Scalar>(), ad_model->get_ntau(),
           ad_model->get_nu(), ad_model->get_nr(), ad_model->get_ng(),
           ad_model->get_nh(), ad_model->get_ng_T(), ad_model->get_nh_T(),
           ad_model->get_np()),
      model_(
          std::dynamic_pointer_cast<Base>(ad_model->template cast<Scalar>())),
      Y1fun_name_("observer_calc"),
      Y1Tfun_name_("observer_calc_T"),
      Y2fun_name_("observer_calcDiff"),
      Y2Tfun_name_("observer_calcDiff_T"),
      Y2Costfun_name_("observer_calcDiff_cost"),
      Y2CostTfun_name_("observer_calcDiff_cost_T"),
      lib_fname_(lib_fname),
      ad_calc_(std::make_unique<ADFun>()),
      ad_calc_T_(std::make_unique<ADFun>()),
      ad_calcDiff_(std::make_unique<ADFun>()),
      ad_calcDiff_T_(std::make_unique<ADFun>()),
      ad_calcDiffCost_(std::make_unique<ADFun>()),
      ad_calcDiffCost_T_(std::make_unique<ADFun>()) {
  loadLib(lib_fname_);
  nX_ = calcFun_->Domain();
  nX_T_ = calcFun_T_->Domain();
  nY1_ = calcFun_->Range();
  nY1_T_ = calcFun_T_->Range();
  nX2_ = calcDiffFun_->Domain();
  nX2_T_ = calcDiffFun_T_->Domain();
  nY2_ = calcDiffFun_->Range();
  nY2_T_ = calcDiffFun_T_->Range();
  nYh_ = calcDiffCostFun_->Range();
  nYh_T_ = calcDiffCostFun_T_->Range();
  np_ = ad_model->get_np();
  nenv_ = nX_T_ - state_->get_nx() - np_ - ntau_;
  ad_X_.resize(nX_);
  ad_X_T_.resize(nX_T_);
  ad_X2_.resize(nX2_);
  ad_X2_T_.resize(nX2_T_);
  ad_Y1_.resize(nY1_);
  ad_Y1_T_.resize(nY1_T_);
  ad_Y2_.resize(nY2_);
  ad_Y2_T_.resize(nY2_T_);
  autodiff_ = nX2_ != nX_;
  if (autodiff_ && (!calcDiffFun_->isJacobianAvailable() || !calcDiffCostFun_ ||
                    !calcDiffCostFun_T_)) {
    throw_pretty(
        "The code generated library contains a stale autodiff observer "
        "calcDiff tape; please regenerate it");
  }
  wCostHess_ = VectorXs::Zero(nY2_);
  wCostHess_(0) = Scalar(1.);
  wCostHess_T_ = VectorXs::Zero(nY2_T_);
  wCostHess_T_(0) = Scalar(1.);
  wCostHessScalar_ = VectorXs::Ones(1);
  wCostHessScalar_T_ = VectorXs::Ones(1);
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::initLib() {
  START_PROFILER("ObserverModelCodeGen::initLib");
  const CodegenEigenThreadGuard eigen_thread_guard(1);
  recordCalc();
  calcCG_ =
      std::unique_ptr<CSourceGen>(new CSourceGen(*ad_calc_.get(), Y1fun_name_));
  calcCG_->setCreateForwardZero(true);
  calcCG_->setCreateJacobian(false);
  calcCG_->setCreateHessian(false);
  recordCalc_T();
  calcCG_T_ = std::unique_ptr<CSourceGen>(
      new CSourceGen(*ad_calc_T_.get(), Y1Tfun_name_));
  calcCG_T_->setCreateForwardZero(true);
  calcCG_T_->setCreateJacobian(false);
  calcCG_T_->setCreateHessian(false);
  recordCalcDiff();
  calcDiffCG_ = std::unique_ptr<CSourceGen>(
      new CSourceGen(*ad_calcDiff_.get(), Y2fun_name_));
  calcDiffCG_->setCreateForwardZero(!autodiff_);
  calcDiffCG_->setCreateJacobian(autodiff_);
  calcDiffCG_->setCreateHessian(false);
  recordCalcDiff_T();
  calcDiffCG_T_ = std::unique_ptr<CSourceGen>(
      new CSourceGen(*ad_calcDiff_T_.get(), Y2Tfun_name_));
  calcDiffCG_T_->setCreateForwardZero(!autodiff_);
  calcDiffCG_T_->setCreateJacobian(autodiff_);
  calcDiffCG_T_->setCreateHessian(false);
  recordCalcDiffCost();
  calcDiffCostCG_ = std::unique_ptr<CSourceGen>(
      new CSourceGen(*ad_calcDiffCost_.get(), Y2Costfun_name_));
  calcDiffCostCG_->setCreateForwardZero(autodiff_);
  calcDiffCostCG_->setCreateJacobian(false);
  calcDiffCostCG_->setCreateHessian(false);
  recordCalcDiffCost_T();
  calcDiffCostCG_T_ = std::unique_ptr<CSourceGen>(
      new CSourceGen(*ad_calcDiffCost_T_.get(), Y2CostTfun_name_));
  calcDiffCostCG_T_->setCreateForwardZero(autodiff_);
  calcDiffCostCG_T_->setCreateJacobian(false);
  calcDiffCostCG_T_->setCreateHessian(false);
  libCG_ = std::unique_ptr<LibraryCSourceGen>(
      new LibraryCSourceGen(*calcCG_, *calcCG_T_, *calcDiffCG_, *calcDiffCG_T_,
                            *calcDiffCostCG_, *calcDiffCostCG_T_));
  dynLibManager_ = std::unique_ptr<LibraryProcessor>(
      new LibraryProcessor(*libCG_, lib_fname_));
  STOP_PROFILER("ObserverModelCodeGen::initLib");
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::compileLib() {
  START_PROFILER("ObserverModelCodeGen::compileLib");
  if (!dynLibManager_) {
    throw_pretty("The library "
                 << lib_fname_ + SystemInfo::DYNAMIC_LIB_EXTENSION
                 << " should not be compiled again");
  }
  auto splitFlags = [](const std::string& flags) {
    std::istringstream iss(flags);
    std::vector<std::string> out;
    std::string flag;
    while (iss >> flag) {
      out.push_back(flag);
    }
    return out;
  };
  switch (compiler_type_) {
    case GCC: {
      CppAD::cg::GccCompiler<Scalar> compiler(compilerExecutable(GCC));
      std::vector<std::string> compile_flags = compiler.getCompileFlags();
      compile_flags.clear();
      auto extra_flags = splitFlags(compile_options_);
      compile_flags.insert(compile_flags.end(), extra_flags.begin(),
                           extra_flags.end());
      compiler.setCompileFlags(compile_flags);
      dynLibManager_->createDynamicLibrary(compiler, false);
      break;
    }
    case CLANG: {
      CppAD::cg::ClangCompiler<Scalar> compiler(compilerExecutable(CLANG));
      std::vector<std::string> compile_flags = compiler.getCompileFlags();
      compile_flags.clear();
      auto extra_flags = splitFlags(compile_options_);
      compile_flags.insert(compile_flags.end(), extra_flags.begin(),
                           extra_flags.end());
      compiler.setCompileFlags(compile_flags);
      dynLibManager_->createDynamicLibrary(compiler, false);
      break;
    }
  }
  STOP_PROFILER("ObserverModelCodeGen::compileLib");
}

template <typename Scalar>
bool ObserverModelCodeGenTpl<Scalar>::existLib(
    const std::string& lib_fname) const {
  const std::string filename = lib_fname + SystemInfo::DYNAMIC_LIB_EXTENSION;
  std::ifstream file(filename.c_str());
  return file.good();
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::loadLib(const std::string& lib_fname) {
  if (!existLib(lib_fname)) {
    throw_pretty("The library " << lib_fname + SystemInfo::DYNAMIC_LIB_EXTENSION
                                << " doesn't exist");
  }
  const std::string filename = lib_fname + SystemInfo::DYNAMIC_LIB_EXTENSION;
  if (dynLibManager_) {
    const auto it = dynLibManager_->getOptions().find("dlOpenMode");
    if (it == dynLibManager_->getOptions().end()) {
      dynLib_.reset(new LinuxDynamicLib(filename));
    } else {
      int dlOpenMode = std::stoi(it->second);
      dynLib_.reset(new LinuxDynamicLib(filename, dlOpenMode));
    }
  } else {
    dynLib_.reset(new LinuxDynamicLib(filename));
  }
  calcFun_ = dynLib_->model(Y1fun_name_.c_str());
  calcFun_T_ = dynLib_->model(Y1Tfun_name_.c_str());
  calcDiffFun_ = dynLib_->model(Y2fun_name_.c_str());
  calcDiffFun_T_ = dynLib_->model(Y2Tfun_name_.c_str());
  calcDiffCostFun_ = dynLib_->model(Y2Costfun_name_.c_str());
  calcDiffCostFun_T_ = dynLib_->model(Y2CostTfun_name_.c_str());
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::update_p(
    const std::shared_ptr<ActionDataAbstract>& data,
    const Eigen::Ref<const VectorXs>& p) {
  if (static_cast<std::size_t>(p.size()) != np_) {
    throw_pretty(
        "Invalid argument: " << "p has wrong dimension (it should be " +
                                    std::to_string(np_) + ")");
  }
  Data* d = static_cast<Data*>(data.get());
  const std::size_t nx = state_->get_nx();
  d->X.segment(nx + nu_, np_) = p;
  d->X_T.segment(nx, np_) = p;
  d->X2.segment(nx + nu_, np_) = p;
  d->X2_T.segment(nx, np_) = p;
  if (params_ && d->params_data) {
    params_->update(d->params_data, p);
  }
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::update_env(
    const std::shared_ptr<ActionDataAbstract>& data,
    const Eigen::Ref<const VectorXs>& env) {
  if (static_cast<std::size_t>(env.size()) != nenv_) {
    throw_pretty(
        "Invalid argument: " << "env has wrong dimension (it should be " +
                                    std::to_string(nenv_) + ")");
  }
  Data* d = static_cast<Data*>(data.get());
  const std::size_t nx = state_->get_nx();
  d->X.segment(nx + nu_ + np_, nenv_) = env;
  d->X_T.segment(nx + np_, nenv_) = env;
  d->X2.segment(nx + nu_ + np_, nenv_) = env;
  d->X2_T.segment(nx + np_, nenv_) = env;
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::update_tau(
    const Eigen::Ref<const VectorXs>& tau_meas) {
  Base::update_tau(tau_meas);
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::set_params(
    const std::shared_ptr<ActionDataAbstract>& data,
    std::shared_ptr<ParameterManager> params) {
  if (!params) {
    throw_pretty("Invalid argument: " << "params cannot be null");
  }
  if (params->get_np() != np_) {
    throw_pretty(
        "Invalid argument: " << "params has wrong dimension (it should be " +
                                    std::to_string(np_) + ")");
  }
  params_ = params;
  Data* d = static_cast<Data*>(data.get());
  if (d->params_data) {
    update_p(data, d->params_data->params->p);
  } else {
    update_p(data, params_->zero());
  }
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::calc(
    const std::shared_ptr<ActionDataAbstract>& data,
    const Eigen::Ref<const VectorXs>& x, const Eigen::Ref<const VectorXs>& u) {
  START_PROFILER("ObserverModelCodeGen::calc");
  Data* d = static_cast<Data*>(data.get());
  d->ActionDataAbstractTpl<Scalar>::resize(this);
  const std::size_t nx = state_->get_nx();
  d->X.head(nx) = x;
  d->X.segment(nx, nu_) = u;
  syncDataParameters(d);
  d->X.segment(nx + nu_ + np_ + nenv_, ntau_) = tau_meas_;
  START_PROFILER("ObserverModelCodeGen::calc::ForwardZero");
  calcFun_->ForwardZero(d->X, d->Y1);
  STOP_PROFILER("ObserverModelCodeGen::calc::ForwardZero");
  d->set_Y1(this);
  STOP_PROFILER("ObserverModelCodeGen::calc");
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::calc(
    const std::shared_ptr<ActionDataAbstract>& data,
    const Eigen::Ref<const VectorXs>& x) {
  START_PROFILER("ObserverModelCodeGen::calc_T");
  Data* d = static_cast<Data*>(data.get());
  d->g.conservativeResize(ng_T_);
  d->h.conservativeResize(nh_T_);
  const std::size_t nx = state_->get_nx();
  d->X_T.head(nx) = x;
  syncDataParameters(d);
  d->X_T.segment(nx + np_ + nenv_, ntau_) = tau_meas_;
  START_PROFILER("ObserverModelCodeGen::calc_T::ForwardZero");
  calcFun_T_->ForwardZero(d->X_T, d->Y1_T);
  STOP_PROFILER("ObserverModelCodeGen::calc_T::ForwardZero");
  d->xnext = x;
  d->set_Y1_T(this);
  STOP_PROFILER("ObserverModelCodeGen::calc_T");
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::calcDiff(
    const std::shared_ptr<ActionDataAbstract>& data,
    const Eigen::Ref<const VectorXs>& x, const Eigen::Ref<const VectorXs>& u) {
  START_PROFILER("ObserverModelCodeGen::calcDiff");
  Data* d = static_cast<Data*>(data.get());
  d->ActionDataAbstractTpl<Scalar>::resize(this);
  const std::size_t nx = state_->get_nx();
  d->X.head(nx) = x;
  d->X.segment(nx, nu_) = u;
  if (autodiff_) {
    d->X2.head(nx) = x;
    d->X2.segment(nx, nu_) = u;
    d->X2.segment(nX_, state_->get_ndx() + nu_ + np_).setZero();
  }
  syncDataParameters(d);
  d->X.segment(nx + nu_ + np_ + nenv_, ntau_) = tau_meas_;
  if (autodiff_) {
    d->X2.segment(nx + nu_ + np_ + nenv_, ntau_) = tau_meas_;
  }
  if (autodiff_) {
    START_PROFILER("ObserverModelCodeGen::calcDiff::Jacobian");
    d->J2 = calcDiffFun_->Jacobian(d->X2);
    STOP_PROFILER("ObserverModelCodeGen::calcDiff::Jacobian");
    d->set_D2(this);
    START_PROFILER("ObserverModelCodeGen::calcDiff::ForwardZeroCost");
    calcDiffCostFun_->ForwardZero(d->X2, d->Yh);
    STOP_PROFILER("ObserverModelCodeGen::calcDiff::ForwardZeroCost");
    d->set_Yh_hessian(this);
  } else {
    START_PROFILER("ObserverModelCodeGen::calcDiff::ForwardZero");
    calcDiffFun_->ForwardZero(d->X, d->Y2);
    STOP_PROFILER("ObserverModelCodeGen::calcDiff::ForwardZero");
    d->set_Y2(this);
  }
  STOP_PROFILER("ObserverModelCodeGen::calcDiff");
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::calcDiff(
    const std::shared_ptr<ActionDataAbstract>& data,
    const Eigen::Ref<const VectorXs>& x) {
  START_PROFILER("ObserverModelCodeGen::calcDiff_T");
  Data* d = static_cast<Data*>(data.get());
  d->Gx.conservativeResize(ng_T_, state_->get_ndx());
  d->Gp.conservativeResize(ng_T_, np_);
  d->Hx.conservativeResize(nh_T_, state_->get_ndx());
  d->Hp.conservativeResize(nh_T_, np_);
  const std::size_t nx = state_->get_nx();
  d->X_T.head(nx) = x;
  if (autodiff_) {
    d->X2_T.head(nx) = x;
    d->X2_T.segment(nX_T_, state_->get_ndx() + np_).setZero();
  }
  syncDataParameters(d);
  d->X_T.segment(nx + np_ + nenv_, ntau_) = tau_meas_;
  if (autodiff_) {
    d->X2_T.segment(nx + np_ + nenv_, ntau_) = tau_meas_;
  }
  if (autodiff_) {
    START_PROFILER("ObserverModelCodeGen::calcDiff_T::Jacobian");
    d->J2_T = calcDiffFun_T_->Jacobian(d->X2_T);
    STOP_PROFILER("ObserverModelCodeGen::calcDiff_T::Jacobian");
    d->set_D2_T(this);
    START_PROFILER("ObserverModelCodeGen::calcDiff_T::ForwardZeroCost");
    calcDiffCostFun_T_->ForwardZero(d->X2_T, d->Yh_T);
    STOP_PROFILER("ObserverModelCodeGen::calcDiff_T::ForwardZeroCost");
    d->set_Yh_hessian_T(this);
  } else {
    START_PROFILER("ObserverModelCodeGen::calcDiff_T::ForwardZero");
    calcDiffFun_T_->ForwardZero(d->X_T, d->Y2_T);
    STOP_PROFILER("ObserverModelCodeGen::calcDiff_T::ForwardZero");
    d->set_Y2_T(this);
  }
  STOP_PROFILER("ObserverModelCodeGen::calcDiff_T");
}

template <typename Scalar>
std::shared_ptr<ActionDataAbstractTpl<Scalar>>
ObserverModelCodeGenTpl<Scalar>::createData() {
  return createData(std::shared_ptr<ParameterDataManagerTpl<Scalar>>());
}

template <typename Scalar>
std::shared_ptr<ActionDataAbstractTpl<Scalar>>
ObserverModelCodeGenTpl<Scalar>::createData(
    const std::shared_ptr<ParameterDataManagerTpl<Scalar>>& params_data) {
  const std::shared_ptr<ActionDataAbstract>& data =
      std::allocate_shared<Data>(Eigen::aligned_allocator<Data>(), this);
  std::static_pointer_cast<Data>(data)->params_data = params_data;
  enableMultithreading() = true;
  return data;
}

template <typename Scalar>
bool ObserverModelCodeGenTpl<Scalar>::checkData(
    const std::shared_ptr<ActionDataAbstract>& data) {
  return std::dynamic_pointer_cast<Data>(data) != nullptr;
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::syncDataParameters(
    Data* const data) const {
  if (data->params_data == nullptr) {
    return;
  }
  if (data->params_data->params == nullptr ||
      data->params_data->params->np != np_) {
    throw_pretty(
        "Invalid argument: shared parameter data has wrong dimension "
        "(it should be " +
        std::to_string(np_) + ")");
  }
  data->X.segment(state_->get_nx() + nu_, np_) = data->params_data->params->p;
  data->X_T.segment(state_->get_nx(), np_) = data->params_data->params->p;
  data->X2.segment(state_->get_nx() + nu_, np_) = data->params_data->params->p;
  data->X2_T.segment(state_->get_nx(), np_) = data->params_data->params->p;
}

template <typename Scalar>
const std::shared_ptr<ObserverModelAbstractTpl<Scalar>>&
ObserverModelCodeGenTpl<Scalar>::get_model() const {
  return model_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_np() const {
  return np_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_nenv() const {
  return nenv_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_ng() const {
  return ng_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_nh() const {
  return nh_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_ng_T() const {
  return ng_T_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_nh_T() const {
  return nh_T_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::VectorXs&
ObserverModelCodeGenTpl<Scalar>::get_g_lb() const {
  return Base::get_g_lb();
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::VectorXs&
ObserverModelCodeGenTpl<Scalar>::get_g_ub() const {
  return Base::get_g_ub();
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_nX() const {
  return nX_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_nX_T() const {
  return nX_T_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_nX2() const {
  return nX2_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_nX2_T() const {
  return nX2_T_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_nY1() const {
  return nY1_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_nY1_T() const {
  return nY1_T_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_nY2() const {
  return nY2_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_nY2_T() const {
  return nY2_T_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_nYh() const {
  return nYh_;
}

template <typename Scalar>
std::size_t ObserverModelCodeGenTpl<Scalar>::get_nYh_T() const {
  return nYh_T_;
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::print(std::ostream& os) const {
  if (model_) {
    model_->print(os);
  } else {
    os << "ObserverModelCodeGen";
  }
}

template <typename Scalar>
ObserverModelCodeGenTpl<Scalar>::ObserverModelCodeGenTpl()
    : Base(std::shared_ptr<StateAbstractTpl<Scalar>>(), 0, 0),
      model_(nullptr),
      np_(0),
      nenv_(0),
      lib_fname_(""),
      compiler_type_(defaultCompilerType()),
      compile_options_("-O -ffast-math -march=native"),
      updateParams_(EmptyParamsEnv) {}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::recordCalc() {
  const std::size_t nx = state_->get_nx();
  CppAD::Independent(ad_X_);
  ad_model_->update_tau(ad_X_.segment(nx + nu_ + np_ + nenv_, ntau_));
  recordParams(ad_X_.segment(nx + nu_, np_),
               ad_X_.segment(nx + nu_ + np_, nenv_));
  ad_model_->calc(ad_data_, ad_X_.head(nx), ad_X_.segment(nx, nu_));
  tapeCalcOutput();
  ad_calc_->Dependent(ad_X_, ad_Y1_);
  ad_calc_->optimize("no_compare_op");
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::recordCalc_T() {
  const std::size_t nx = state_->get_nx();
  CppAD::Independent(ad_X_T_);
  ad_model_->update_tau(ad_X_T_.segment(nx + np_ + nenv_, ntau_));
  recordParams(ad_X_T_.segment(nx, np_), ad_X_T_.segment(nx + np_, nenv_));
  ad_model_->calc(ad_data_, ad_X_T_.head(nx));
  tapeCalcOutput_T();
  ad_calc_T_->Dependent(ad_X_T_, ad_Y1_T_);
  ad_calc_T_->optimize("no_compare_op");
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::recordCalcDiff() {
  const std::size_t nx = state_->get_nx();
  const std::size_t ndx = state_->get_ndx();
  if (autodiff_) {
    CppAD::Independent(ad_X2_);
    const Eigen::VectorBlock<ADVectorXs> dx = ad_X2_.segment(nX_, ndx);
    const Eigen::VectorBlock<ADVectorXs> du = ad_X2_.segment(nX_ + ndx, nu_);
    const Eigen::VectorBlock<ADVectorXs> dp =
        ad_X2_.segment(nX_ + ndx + nu_, np_);

    ADVectorXs x_pert(nx);
    ADVectorXs u_pert(nu_);
    ADVectorXs p_pert(np_);
    ADVectorXs dxnext(ndx);

    ad_model_->update_tau(ad_X2_.segment(nx + nu_ + np_ + nenv_, ntau_));
    recordParams(ad_data_, ad_X2_.segment(nx + nu_, np_),
                 ad_X2_.segment(nx + nu_ + np_, nenv_));
    ad_model_->calc(ad_data_, ad_X2_.head(nx), ad_X2_.segment(nx, nu_));
    ad_model_->get_state()->integrate(ad_X2_.head(nx), dx, x_pert);
    u_pert = ad_X2_.segment(nx, nu_) + du;
    p_pert = ad_X2_.segment(nx + nu_, np_) + dp;
    ad_model_->update_tau(ad_X2_.segment(nx + nu_ + np_ + nenv_, ntau_));
    recordParams(ad_data_pert_, p_pert, ad_X2_.segment(nx + nu_ + np_, nenv_));
    ad_model_->calc(ad_data_pert_, x_pert, u_pert);
    ad_model_->get_state()->diff(ad_data_->xnext, ad_data_pert_->xnext, dxnext);

    Eigen::DenseIndex it_Y2 = 0;
    ad_Y2_[it_Y2] = ad_data_pert_->cost;
    it_Y2 += 1;
    ad_Y2_.segment(it_Y2, ndx) = dxnext;
    it_Y2 += ndx;
    ad_Y2_.segment(it_Y2, ng_) = ad_data_pert_->g.head(ng_);
    it_Y2 += ng_;
    ad_Y2_.segment(it_Y2, nh_) = ad_data_pert_->h.head(nh_);
    it_Y2 += nh_;
    ad_Y2_[it_Y2] = ad_data_pert_->dissipative_E[0];
    ad_calcDiff_->Dependent(ad_X2_, ad_Y2_);
  } else {
    CppAD::Independent(ad_X_);
    ad_model_->update_tau(ad_X_.segment(nx + nu_ + np_ + nenv_, ntau_));
    recordParams(ad_X_.segment(nx + nu_, np_),
                 ad_X_.segment(nx + nu_ + np_, nenv_));
    ad_model_->calc(ad_data_, ad_X_.head(nx), ad_X_.segment(nx, nu_));
    ad_model_->calcDiff(ad_data_, ad_X_.head(nx), ad_X_.segment(nx, nu_));
    tapeCalcDiffDirectOutput(ad_Y2_);
    ad_calcDiff_->Dependent(ad_X_, ad_Y2_);
  }
  ad_calcDiff_->optimize("no_compare_op");
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::recordCalcDiff_T() {
  const std::size_t nx = state_->get_nx();
  const std::size_t ndx = state_->get_ndx();
  if (autodiff_) {
    CppAD::Independent(ad_X2_T_);
    const Eigen::VectorBlock<ADVectorXs> dx = ad_X2_T_.segment(nX_T_, ndx);
    const Eigen::VectorBlock<ADVectorXs> dp =
        ad_X2_T_.segment(nX_T_ + ndx, np_);

    ADVectorXs x_pert(nx);
    ADVectorXs p_pert(np_);
    ADVectorXs dxnext(ndx);

    ad_model_->update_tau(ad_X2_T_.segment(nx + np_ + nenv_, ntau_));
    recordParams(ad_data_, ad_X2_T_.segment(nx, np_),
                 ad_X2_T_.segment(nx + np_, nenv_));
    ad_model_->calc(ad_data_, ad_X2_T_.head(nx));
    ad_model_->get_state()->integrate(ad_X2_T_.head(nx), dx, x_pert);
    p_pert = ad_X2_T_.segment(nx, np_) + dp;
    ad_model_->update_tau(ad_X2_T_.segment(nx + np_ + nenv_, ntau_));
    recordParams(ad_data_pert_, p_pert, ad_X2_T_.segment(nx + np_, nenv_));
    ad_model_->calc(ad_data_pert_, x_pert);
    ad_model_->get_state()->diff(ad_X2_T_.head(nx), x_pert, dxnext);

    Eigen::DenseIndex it_Y2 = 0;
    ad_Y2_T_[it_Y2] = ad_data_pert_->cost;
    it_Y2 += 1;
    ad_Y2_T_.segment(it_Y2, ndx) = dxnext;
    it_Y2 += ndx;
    ad_Y2_T_.segment(it_Y2, ng_T_) = ad_data_pert_->g.head(ng_T_);
    it_Y2 += ng_T_;
    ad_Y2_T_.segment(it_Y2, nh_T_) = ad_data_pert_->h.head(nh_T_);
    it_Y2 += nh_T_;
    ad_Y2_T_[it_Y2] = ad_data_pert_->dissipative_E[0];
    ad_calcDiff_T_->Dependent(ad_X2_T_, ad_Y2_T_);
  } else {
    CppAD::Independent(ad_X_T_);
    ad_model_->update_tau(ad_X_T_.segment(nx + np_ + nenv_, ntau_));
    recordParams(ad_X_T_.segment(nx, np_), ad_X_T_.segment(nx + np_, nenv_));
    ad_model_->calc(ad_data_, ad_X_T_.head(nx));
    ad_model_->calcDiff(ad_data_, ad_X_T_.head(nx));
    tapeCalcDiffDirectOutput_T(ad_Y2_T_);
    ad_calcDiff_T_->Dependent(ad_X_T_, ad_Y2_T_);
  }
  ad_calcDiff_T_->optimize("no_compare_op");
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::tapeCalcDiffHessianOutput(ADVectorXs& y) {
  const std::size_t ndx = state_->get_ndx();
  Eigen::DenseIndex it_y = 0;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, ndx, ndx) = ad_data_->Lxx;
  it_y += ndx * ndx;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, ndx, nu_) = ad_data_->Lxu;
  it_y += ndx * nu_;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, nu_, nu_) = ad_data_->Luu;
  it_y += nu_ * nu_;
  Eigen::Map<ADMatrixXs> Lpp_map(y.data() + it_y, np_, np_);
  if (static_cast<std::size_t>(ad_data_->Lpp.rows()) == np_ &&
      static_cast<std::size_t>(ad_data_->Lpp.cols()) == np_) {
    Lpp_map = ad_data_->Lpp;
  } else {
    Lpp_map.setZero();
  }
  it_y += np_ * np_;
  Eigen::Map<ADMatrixXs> Lpx_map(y.data() + it_y, np_, ndx);
  if (static_cast<std::size_t>(ad_data_->Lpx.rows()) == np_) {
    Lpx_map = ad_data_->Lpx;
  } else {
    Lpx_map.setZero();
  }
  it_y += np_ * ndx;
  Eigen::Map<ADMatrixXs> Lpu_map(y.data() + it_y, np_, nu_);
  if (static_cast<std::size_t>(ad_data_->Lpu.rows()) == np_) {
    Lpu_map = ad_data_->Lpu;
  } else {
    Lpu_map.setZero();
  }
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::tapeCalcDiffHessianOutput_T(
    ADVectorXs& y) {
  const std::size_t ndx = state_->get_ndx();
  Eigen::DenseIndex it_y = 0;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, ndx, ndx) = ad_data_->Lxx;
  it_y += ndx * ndx;
  Eigen::Map<ADMatrixXs> Lpp_map(y.data() + it_y, np_, np_);
  if (static_cast<std::size_t>(ad_data_->Lpp.rows()) == np_ &&
      static_cast<std::size_t>(ad_data_->Lpp.cols()) == np_) {
    Lpp_map = ad_data_->Lpp;
  } else {
    Lpp_map.setZero();
  }
  it_y += np_ * np_;
  Eigen::Map<ADMatrixXs> Lpx_map(y.data() + it_y, np_, ndx);
  if (static_cast<std::size_t>(ad_data_->Lpx.rows()) == np_) {
    Lpx_map = ad_data_->Lpx;
  } else {
    Lpx_map.setZero();
  }
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::recordCalcDiffCost() {
  const std::size_t nx = state_->get_nx();
  CppAD::Independent(ad_X2_);
  ADVectorXs y(nYh_);
  y.setZero();
  ad_model_->update_tau(ad_X2_.segment(nx + nu_ + np_ + nenv_, ntau_));
  recordParams(ad_X2_.segment(nx + nu_, np_),
               ad_X2_.segment(nx + nu_ + np_, nenv_));
  ad_model_->calc(ad_data_, ad_X2_.head(nx), ad_X2_.segment(nx, nu_));
  ad_model_->calcDiff(ad_data_, ad_X2_.head(nx), ad_X2_.segment(nx, nu_));
  if (autodiff_) {
    tapeCalcDiffDirectOutput(y);
  } else {
    tapeCalcDiffHessianOutput(y);
  }
  ad_calcDiffCost_->Dependent(ad_X2_, y);
  ad_calcDiffCost_->optimize("no_compare_op");
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::recordCalcDiffCost_T() {
  const std::size_t nx = state_->get_nx();
  CppAD::Independent(ad_X2_T_);
  ADVectorXs y(nYh_T_);
  y.setZero();
  ad_model_->update_tau(ad_X2_T_.segment(nx + np_ + nenv_, ntau_));
  recordParams(ad_X2_T_.segment(nx, np_), ad_X2_T_.segment(nx + np_, nenv_));
  ad_model_->calc(ad_data_, ad_X2_T_.head(nx));
  ad_model_->calcDiff(ad_data_, ad_X2_T_.head(nx));
  if (autodiff_) {
    tapeCalcDiffDirectOutput_T(y);
  } else {
    tapeCalcDiffHessianOutput_T(y);
  }
  ad_calcDiffCost_T_->Dependent(ad_X2_T_, y);
  ad_calcDiffCost_T_->optimize("no_compare_op");
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::recordParams(
    const Eigen::Ref<const ADVectorXs>& p,
    const Eigen::Ref<const ADVectorXs>& env) {
  recordParams(ad_data_, p, env);
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::recordParams(
    const std::shared_ptr<ADObserverDataAbstract>& data,
    const Eigen::Ref<const ADVectorXs>& p,
    const Eigen::Ref<const ADVectorXs>& env) {
  if (np_ > 0 && ad_model_->get_np() == np_) {
    ad_model_->update_p(data, p);
  }
  updateParams_(ad_model_, nenv_ == 0u ? p : env);
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::setupADParameters() {
  const std::shared_ptr<IntegratedObserverModelAbstractTpl<ADScalar>>
      integrated = std::dynamic_pointer_cast<
          IntegratedObserverModelAbstractTpl<ADScalar>>(ad_model_);
  const std::shared_ptr<ParameterManagerTpl<ADScalar>> ad_params =
      integrated != nullptr ? integrated->get_params()
                            : std::shared_ptr<ParameterManagerTpl<ADScalar>>();
  ad_data_ =
      std::static_pointer_cast<ADObserverDataAbstract>(ad_model_->createData(
          ad_params != nullptr ? ad_params->createData() : nullptr));
  ad_data_pert_ =
      std::static_pointer_cast<ADObserverDataAbstract>(ad_model_->createData(
          ad_params != nullptr ? ad_params->createData() : nullptr));
  if (np_ != 0 && ad_model_->get_np() == np_ && ad_params != nullptr) {
    ad_model_->set_params(ad_data_, ad_params);
    ad_model_->set_params(ad_data_pert_, ad_params);
  }
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::tapeCalcOutput() {
  Eigen::DenseIndex it_Y1 = 0;
  ad_Y1_[it_Y1] = ad_data_->cost;
  it_Y1 += 1;
  ad_Y1_.segment(it_Y1, state_->get_nx()) = ad_data_->xnext;
  it_Y1 += state_->get_nx();
  ad_Y1_.segment(it_Y1, ng_) = ad_data_->g;
  it_Y1 += ng_;
  ad_Y1_.segment(it_Y1, nh_) = ad_data_->h;
  it_Y1 += nh_;
  ad_Y1_[it_Y1] = ad_data_->dissipative_E[0];
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::tapeCalcOutput_T() {
  Eigen::DenseIndex it_Y1 = 0;
  ad_Y1_T_[it_Y1] = ad_data_->cost;
  it_Y1 += 1;
  ad_Y1_T_.segment(it_Y1, ng_T_) = ad_data_->g;
  it_Y1 += ng_T_;
  ad_Y1_T_.segment(it_Y1, nh_T_) = ad_data_->h;
  it_Y1 += nh_T_;
  ad_Y1_T_[it_Y1] = ad_data_->dissipative_E[0];
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::tapeCalcDiffOutput() {
  const std::size_t ndx = state_->get_ndx();
  Eigen::DenseIndex it_Y2 = 0;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, ndx, ndx) = ad_data_->Fx;
  it_Y2 += ndx * ndx;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, ndx, nu_) = ad_data_->Fu;
  it_Y2 += ndx * nu_;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, ndx, np_) = ad_data_->Fp;
  it_Y2 += ndx * np_;
  Eigen::Map<ADVectorXs>(ad_Y2_.data() + it_Y2, ndx) = ad_data_->Lx;
  it_Y2 += ndx;
  Eigen::Map<ADVectorXs>(ad_Y2_.data() + it_Y2, nu_) = ad_data_->Lu;
  it_Y2 += nu_;
  Eigen::Map<ADVectorXs>(ad_Y2_.data() + it_Y2, np_) = ad_data_->Lp;
  it_Y2 += np_;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, ndx, ndx) = ad_data_->Lxx;
  it_Y2 += ndx * ndx;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, ndx, nu_) = ad_data_->Lxu;
  it_Y2 += ndx * nu_;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, nu_, nu_) = ad_data_->Luu;
  it_Y2 += nu_ * nu_;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, np_, np_) = ad_data_->Lpp;
  it_Y2 += np_ * np_;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, np_, ndx) = ad_data_->Lpx;
  it_Y2 += np_ * ndx;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, np_, nu_) = ad_data_->Lpu;
  it_Y2 += np_ * nu_;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, ng_, ndx) = ad_data_->Gx;
  it_Y2 += ng_ * ndx;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, ng_, nu_) = ad_data_->Gu;
  it_Y2 += ng_ * nu_;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, ng_, np_) = ad_data_->Gp;
  it_Y2 += ng_ * np_;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, nh_, ndx) = ad_data_->Hx;
  it_Y2 += nh_ * ndx;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, nh_, nu_) = ad_data_->Hu;
  it_Y2 += nh_ * nu_;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, nh_, np_) = ad_data_->Hp;
  it_Y2 += nh_ * np_;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, 1, ndx) = ad_data_->Ex;
  it_Y2 += ndx;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, 1, nu_) = ad_data_->Eu;
  it_Y2 += nu_;
  Eigen::Map<ADMatrixXs>(ad_Y2_.data() + it_Y2, 1, np_) = ad_data_->Ep;
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::tapeCalcDiffOutput_T() {
  const std::size_t ndx = state_->get_ndx();
  Eigen::DenseIndex it_Y2 = 0;
  Eigen::Map<ADVectorXs>(ad_Y2_T_.data() + it_Y2, ndx) = ad_data_->Lx;
  it_Y2 += ndx;
  Eigen::Map<ADVectorXs>(ad_Y2_T_.data() + it_Y2, np_) = ad_data_->Lp;
  it_Y2 += np_;
  Eigen::Map<ADMatrixXs>(ad_Y2_T_.data() + it_Y2, ndx, ndx) = ad_data_->Lxx;
  it_Y2 += ndx * ndx;
  Eigen::Map<ADMatrixXs>(ad_Y2_T_.data() + it_Y2, np_, np_) = ad_data_->Lpp;
  it_Y2 += np_ * np_;
  Eigen::Map<ADMatrixXs>(ad_Y2_T_.data() + it_Y2, np_, ndx) = ad_data_->Lpx;
  it_Y2 += np_ * ndx;
  Eigen::Map<ADMatrixXs>(ad_Y2_T_.data() + it_Y2, ng_T_, ndx) = ad_data_->Gx;
  it_Y2 += ng_T_ * ndx;
  Eigen::Map<ADMatrixXs>(ad_Y2_T_.data() + it_Y2, ng_T_, np_) = ad_data_->Gp;
  it_Y2 += ng_T_ * np_;
  Eigen::Map<ADMatrixXs>(ad_Y2_T_.data() + it_Y2, nh_T_, ndx) = ad_data_->Hx;
  it_Y2 += nh_T_ * ndx;
  Eigen::Map<ADMatrixXs>(ad_Y2_T_.data() + it_Y2, nh_T_, np_) = ad_data_->Hp;
  it_Y2 += nh_T_ * np_;
  Eigen::Map<ADMatrixXs>(ad_Y2_T_.data() + it_Y2, 1, ndx) = ad_data_->Ex;
  it_Y2 += ndx;
  Eigen::Map<ADMatrixXs>(ad_Y2_T_.data() + it_Y2, 1, np_) = ad_data_->Ep;
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::tapeCalcDiffDirectOutput(ADVectorXs& y) {
  const std::size_t ndx = state_->get_ndx();
  Eigen::DenseIndex it_y = 0;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, ndx, ndx) = ad_data_->Fx;
  it_y += ndx * ndx;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, ndx, nu_) = ad_data_->Fu;
  it_y += ndx * nu_;
  Eigen::Map<ADMatrixXs> Fp_map(y.data() + it_y, ndx, np_);
  if (static_cast<std::size_t>(ad_data_->Fp.cols()) == np_) {
    Fp_map = ad_data_->Fp;
  } else {
    Fp_map.setZero();
  }
  it_y += ndx * np_;
  Eigen::Map<ADVectorXs>(y.data() + it_y, ndx) = ad_data_->Lx;
  it_y += ndx;
  Eigen::Map<ADVectorXs>(y.data() + it_y, nu_) = ad_data_->Lu;
  it_y += nu_;
  Eigen::Map<ADVectorXs> Lp_map(y.data() + it_y, np_);
  if (static_cast<std::size_t>(ad_data_->Lp.size()) == np_) {
    Lp_map = ad_data_->Lp;
  } else {
    Lp_map.setZero();
  }
  it_y += np_;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, ndx, ndx) = ad_data_->Lxx;
  it_y += ndx * ndx;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, ndx, nu_) = ad_data_->Lxu;
  it_y += ndx * nu_;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, nu_, nu_) = ad_data_->Luu;
  it_y += nu_ * nu_;
  Eigen::Map<ADMatrixXs> Lpp_map(y.data() + it_y, np_, np_);
  if (static_cast<std::size_t>(ad_data_->Lpp.rows()) == np_ &&
      static_cast<std::size_t>(ad_data_->Lpp.cols()) == np_) {
    Lpp_map = ad_data_->Lpp;
  } else {
    Lpp_map.setZero();
  }
  it_y += np_ * np_;
  Eigen::Map<ADMatrixXs> Lpx_map(y.data() + it_y, np_, ndx);
  if (static_cast<std::size_t>(ad_data_->Lpx.rows()) == np_) {
    Lpx_map = ad_data_->Lpx;
  } else {
    Lpx_map.setZero();
  }
  it_y += np_ * ndx;
  Eigen::Map<ADMatrixXs> Lpu_map(y.data() + it_y, np_, nu_);
  if (static_cast<std::size_t>(ad_data_->Lpu.rows()) == np_) {
    Lpu_map = ad_data_->Lpu;
  } else {
    Lpu_map.setZero();
  }
  it_y += np_ * nu_;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, ng_, ndx) = ad_data_->Gx;
  it_y += ng_ * ndx;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, ng_, nu_) = ad_data_->Gu;
  it_y += ng_ * nu_;
  Eigen::Map<ADMatrixXs> Gp_map(y.data() + it_y, ng_, np_);
  if (static_cast<std::size_t>(ad_data_->Gp.cols()) == np_) {
    Gp_map = ad_data_->Gp;
  } else {
    Gp_map.setZero();
  }
  it_y += ng_ * np_;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, nh_, ndx) = ad_data_->Hx;
  it_y += nh_ * ndx;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, nh_, nu_) = ad_data_->Hu;
  it_y += nh_ * nu_;
  Eigen::Map<ADMatrixXs> Hp_map(y.data() + it_y, nh_, np_);
  if (static_cast<std::size_t>(ad_data_->Hp.cols()) == np_) {
    Hp_map = ad_data_->Hp;
  } else {
    Hp_map.setZero();
  }
  it_y += nh_ * np_;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, 1, ndx) = ad_data_->Ex;
  it_y += ndx;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, 1, nu_) = ad_data_->Eu;
  it_y += nu_;
  Eigen::Map<ADMatrixXs> Ep_map(y.data() + it_y, 1, np_);
  if (static_cast<std::size_t>(ad_data_->Ep.cols()) == np_) {
    Ep_map = ad_data_->Ep;
  } else {
    Ep_map.setZero();
  }
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::tapeCalcDiffDirectOutput_T(
    ADVectorXs& y) {
  const std::size_t ndx = state_->get_ndx();
  Eigen::DenseIndex it_y = 0;
  Eigen::Map<ADVectorXs>(y.data() + it_y, ndx) = ad_data_->Lx;
  it_y += ndx;
  Eigen::Map<ADVectorXs> Lp_map(y.data() + it_y, np_);
  if (static_cast<std::size_t>(ad_data_->Lp.size()) == np_) {
    Lp_map = ad_data_->Lp;
  } else {
    Lp_map.setZero();
  }
  it_y += np_;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, ndx, ndx) = ad_data_->Lxx;
  it_y += ndx * ndx;
  Eigen::Map<ADMatrixXs> Lpp_map(y.data() + it_y, np_, np_);
  if (static_cast<std::size_t>(ad_data_->Lpp.rows()) == np_ &&
      static_cast<std::size_t>(ad_data_->Lpp.cols()) == np_) {
    Lpp_map = ad_data_->Lpp;
  } else {
    Lpp_map.setZero();
  }
  it_y += np_ * np_;
  Eigen::Map<ADMatrixXs> Lpx_map(y.data() + it_y, np_, ndx);
  if (static_cast<std::size_t>(ad_data_->Lpx.rows()) == np_) {
    Lpx_map = ad_data_->Lpx;
  } else {
    Lpx_map.setZero();
  }
  it_y += np_ * ndx;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, ng_T_, ndx) = ad_data_->Gx;
  it_y += ng_T_ * ndx;
  Eigen::Map<ADMatrixXs> Gp_map(y.data() + it_y, ng_T_, np_);
  if (static_cast<std::size_t>(ad_data_->Gp.cols()) == np_) {
    Gp_map = ad_data_->Gp;
  } else {
    Gp_map.setZero();
  }
  it_y += ng_T_ * np_;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, nh_T_, ndx) = ad_data_->Hx;
  it_y += nh_T_ * ndx;
  Eigen::Map<ADMatrixXs> Hp_map(y.data() + it_y, nh_T_, np_);
  if (static_cast<std::size_t>(ad_data_->Hp.cols()) == np_) {
    Hp_map = ad_data_->Hp;
  } else {
    Hp_map.setZero();
  }
  it_y += nh_T_ * np_;
  Eigen::Map<ADMatrixXs>(y.data() + it_y, 1, ndx) = ad_data_->Ex;
  it_y += ndx;
  Eigen::Map<ADMatrixXs> Ep_map(y.data() + it_y, 1, np_);
  if (static_cast<std::size_t>(ad_data_->Ep.cols()) == np_) {
    Ep_map = ad_data_->Ep;
  } else {
    Ep_map.setZero();
  }
}

template <typename Scalar>
typename ObserverModelCodeGenTpl<Scalar>::ParamsEnvironment
ObserverModelCodeGenTpl<Scalar>::makeStateObservationEnvironment(
    const std::string& state_observation_cost, const std::string& weight_cost) {
  return [state_observation_cost, weight_cost](
             std::shared_ptr<ADBase> ad_model,
             const Eigen::Ref<const ADVectorXs>& env) {
    const std::shared_ptr<IntegratedObserverModelAbstractTpl<ADScalar>>
        integrated = std::dynamic_pointer_cast<
            IntegratedObserverModelAbstractTpl<ADScalar>>(ad_model);
    if (!integrated) {
      throw_pretty(
          "Invalid argument: state-observation environment updater requires "
          "an integrated observer model");
    }
    const std::shared_ptr<CostModelSumTpl<ADScalar>>& costs =
        integrated->get_costs();
    typename CostModelSumTpl<ADScalar>::CostModelContainer::const_iterator it =
        costs->get_costs().find(state_observation_cost);
    if (it == costs->get_costs().end()) {
      throw_pretty("Invalid argument: state-observation cost '"
                   << state_observation_cost << "' was not found");
    }
    const std::shared_ptr<ResidualModelStateTpl<ADScalar>> residual =
        std::dynamic_pointer_cast<ResidualModelStateTpl<ADScalar>>(
            it->second->cost->get_residual());
    if (!residual) {
      throw_pretty("Invalid argument: cost '"
                   << state_observation_cost
                   << "' does not wrap a ResidualModelState");
    }
    const std::size_t nx = residual->get_state()->get_nx();
    const std::size_t expected = nx + (weight_cost.empty() ? 0u : 1u);
    const std::size_t env_size = static_cast<std::size_t>(env.size());
    if (env_size != expected && env_size != expected + 1u) {
      throw_pretty("Invalid argument: environment vector has wrong dimension ("
                   << env.size() << " provided - it should be " << expected
                   << " or " << expected + 1u << ")");
    }
    residual->set_reference(env.head(nx));
    if (!weight_cost.empty()) {
      typename CostModelSumTpl<ADScalar>::CostModelContainer::const_iterator
          weight_it = costs->get_costs().find(weight_cost);
      if (weight_it == costs->get_costs().end()) {
        throw_pretty("Invalid argument: weight cost '" << weight_cost
                                                       << "' was not found");
      }
      weight_it->second->weight = env[nx];
    }
    if (env_size == expected + 1u) {
      typename CostModelSumTpl<ADScalar>::CostModelContainer::const_iterator
          power_it = costs->get_costs().find("powerObs");
      if (power_it == costs->get_costs().end()) {
        throw_pretty(
            "Invalid argument: power observation cost 'powerObs' was "
            "not found");
      }
      const std::shared_ptr<ResidualModelPowerTpl<ADScalar>> power_residual =
          std::dynamic_pointer_cast<ResidualModelPowerTpl<ADScalar>>(
              power_it->second->cost->get_residual());
      if (!power_residual) {
        throw_pretty(
            "Invalid argument: cost 'powerObs' does not wrap a "
            "ResidualModelPower");
      }
      power_residual->set_reference(env[expected]);
    }
  };
}

template <typename Scalar>
void ObserverModelCodeGenTpl<Scalar>::EmptyParamsEnv(
    std::shared_ptr<ADBase>, const Eigen::Ref<const ADVectorXs>&) {}

}  // namespace crocoddyl

#endif  // CROCODDYL_WITH_CODEGEN

#endif  // CROCODDYL_CORE_CODEGEN_OBSERVER_HXX_
