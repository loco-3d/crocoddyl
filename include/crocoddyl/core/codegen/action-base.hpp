...
public:
  ...
  void recordCalc() {
    ...
    ad_model->calc(ad_data, ad_X.head(ad_model->get_state()->get_nx()),
                   ad_X.tail(ad_model->get_nu()));
    collect_calcout();
  ...
  void recordCalcDiff() {
    ...
    ad_model->calc(ad_data, ad_X2.head(ad_model->get_state()->get_nx()),
                   ad_X2.tail(ad_model->get_nu()));
    ad_model->calcDiff(ad_data, ad_X2.head(ad_model->get_state()->get_nx()),
                       ad_X2.tail(ad_model->get_nu()));
  ...
  void initLib() {
    ...
    calcgen_ptr = std::unique_ptr<CppAD::cg::ModelCSourceGen<Scalar> >(new CppAD::cg::ModelCSourceGen<Scalar>(ad_calc, function_name_calc));
    calcgen_ptr->setCreateForwardZero(true);
    calcgen_ptr->setCreateJacobian(false);

    // generates source code
    recordCalcDiff();
    calcDiffgen_ptr = std::unique_ptr<CppAD::cg::ModelCSourceGen<Scalar> >(new CppAD::cg::ModelCSourceGen<Scalar>(ad_calcDiff, function_name_calcDiff));
    calcDiffgen_ptr->setCreateForwardZero(true);
    calcDiffgen_ptr->setCreateJacobian(false);


    libcgen_ptr = std::unique_ptr<CppAD::cg::ModelLibraryCSourceGen<Scalar> >(new CppAD::cg::ModelLibraryCSourceGen<Scalar>(*calcgen_ptr, *calcDiffgen_ptr));
  ...
  void calc(const boost::shared_ptr<ActionDataAbstract>& data,
    ...
    ActionDataCodeGenTpl<Scalar>* d =
      static_cast<ActionDataCodeGenTpl<Scalar>*>(data.get());
...
struct ActionDataCodeGenTpl : public ActionDataAbstractTpl<_Scalar> {
  ...
  explicit ActionDataCodeGenTpl(Model<Scalar>* const model)
      : Base(model),
        xu(model->get_state()->get_nx() + model->get_nu()),
        calcout(model->get_state()->get_nx() + 1) {
    xu.setZero();
    calcout.setZero();
