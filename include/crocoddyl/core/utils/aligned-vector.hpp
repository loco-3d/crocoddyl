...
// Adapted from https://github.com/stack-of-tasks/pinocchio/blob/devel/src/container/aligned-vector.hpp
//

#ifndef CROCODDYL_CORE_UTILS_ALIGNED_VECTOR_HPP_
#define CROCODDYL_CORE_UTILS_ALIGNED_VECTOR_HPP_

#include <vector>
#include <Eigen/StdVector>

// #define CROCODDYL_ALIGNED_STD_VECTOR(Type) ::crocoddyl::aligned_vector<Type>
//  std::vector<Type,Eigen::aligned_allocator<Type> >

namespace crocoddyl {
///
/// \brief Specialization of an std::vector with an aligned allocator. This specialization might be used when the type
/// T is or contains some Eigen members.
///
...
struct aligned_vector : public std::vector<T, Eigen::aligned_allocator<T> > {
  ...
  explicit aligned_vector(size_type num, const value_type& val = value_type()) : vector_base(num, val) {}
...
#endif  // ifndef CROCODDYL_CORE_UTILS_ALIGNED_VECTOR_HPP_
