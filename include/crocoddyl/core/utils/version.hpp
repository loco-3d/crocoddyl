...
inline std::string printVersion(const std::string& delimiter = ".") {
  ...
  oss << CROCODDYL_MAJOR_VERSION << delimiter << CROCODDYL_MINOR_VERSION << delimiter << CROCODDYL_PATCH_VERSION;
  return oss.str();
}

///
/// \brief Checks if the current version of Crocoddyl is at least the version provided
...
///        by the input arguments.
///
inline bool checkVersionAtLeast(int major_version, int minor_version, int patch_version) {
  return CROCODDYL_MAJOR_VERSION > major_version ||
         (CROCODDYL_MAJOR_VERSION >= major_version &&
          (CROCODDYL_MINOR_VERSION > minor_version ||
           (CROCODDYL_MINOR_VERSION >= minor_version && CROCODDYL_PATCH_VERSION >= patch_version)));
}
}  // namespace crocoddyl
