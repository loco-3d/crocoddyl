...
template <typename Scalar>
std::vector<typename MathBaseTpl<Scalar>::MatrixXs> StateAbstractTpl<Scalar>::Jintegrate_Js(const Eigen::Ref<const VectorXs>& x, const Eigen::Ref<const VectorXs>& dx, const Jcomponent firstsecond, const AssignmentOp op) {
  MatrixXs Jfirst(ndx_, ndx_), Jsecond(ndx_, ndx_);
  std::vector<MatrixXs> Jacs;
