...
StateAbstractTpl<Scalar>::StateAbstractTpl()
    : nx_(0), ndx_(0), lb_(MathBase::VectorXs::Constant(
                           nx_, -std::numeric_limits<Scalar>::infinity())),
      ub_(MathBase::VectorXs::Constant(
          nx_, std::numeric_limits<Scalar>::infinity())),
      has_limits_(false) {}

template <typename Scalar> StateAbstractTpl<Scalar>::~StateAbstractTpl() {}

template <typename Scalar>
typename MathBaseTpl<Scalar>::VectorXs
StateAbstractTpl<Scalar>::diff_dx(const Eigen::Ref<const VectorXs> &x0,
...
template <typename Scalar>
typename MathBaseTpl<Scalar>::VectorXs
StateAbstractTpl<Scalar>::integrate_x(const Eigen::Ref<const VectorXs> &x,
...
StateAbstractTpl<Scalar>::get_lb() const {
  return lb_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::VectorXs &
StateAbstractTpl<Scalar>::get_ub() const {
  return ub_;
}
...
template <typename Scalar> void StateAbstractTpl<Scalar>::update_has_limits() {
