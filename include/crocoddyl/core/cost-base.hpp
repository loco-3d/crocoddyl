 ...
 * \f$a(\cdot)\f$ and by the residual function \f$\mathbf{r}(\cdot)\f$ as
 * follows: \f[ cost = a(\mathbf{r}(\mathbf{x}, \mathbf{u})), \f] where the
 * residual function depends on the state point \f$\mathbf{x}\in\mathcal{X}\f$,
 * which lies in the state manifold described with a `nq`-tuple, its velocity
 * \f$\dot{\mathbf{x}}\in T_{\mathbf{x}}\mathcal{X}\f$ that belongs to the
 * tangent space with `nv` dimension, and the control input
 * \f$\mathbf{u}\in\mathbb{R}^{nu}\f$. The residual vector is defined by
 * \f$\mathbf{r}\in\mathbb{R}^{nr}\f$ where `nr` describes its dimension in the
 * Euclidean space. On the other hand, the activation function builds a cost
 * value based on the definition of the residual vector. The residual vector has
 * to be specialized in a derived classes.
 *
 ...
 * `calcDiff` builds a linear-quadratic approximation of the cost function with
 * the form: \f$\mathbf{l_x}\in\mathbb{R}^{ndx}\f$,
 ...
 * Hessians, respectively. Additionally, it is important remark that
 * `calcDiff()` computes the derivates using the latest stored values by
 * `calc()`. Thus, we need to run first `calc()`.
 *
 * \sa `StateAbstractTpl`, `ActivationModelAbstractTpl`, `calc()`, `calcDiff()`,
 * `createData()`
 */
template <typename _Scalar> class CostModelAbstractTpl {
public:
   ...
   * it is possible to specialized this function is we need to create additional
   ...
   */
  template <class ReferenceType> void set_reference(ReferenceType ref);

  /**
   * @brief Return the cost reference
   */
  template <class ReferenceType> ReferenceType get_reference() const;
...
template <typename _Scalar> struct CostDataAbstractTpl {
  ...
  CostDataAbstractTpl(Model<Scalar> *const model,
      ...
      : shared(data), activation(model->get_activation()->createData()),
        cost(Scalar(0.)), Lx(model->get_state()->get_ndx()),
        Lu(model->get_nu()),
