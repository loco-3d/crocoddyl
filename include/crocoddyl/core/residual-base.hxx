...
template <typename Scalar>
void ResidualModelAbstractTpl<Scalar>::calcCostDiff(const boost::shared_ptr<CostDataAbstract>& cdata,
                                                    const boost::shared_ptr<ResidualDataAbstract>& rdata,
                                                    const boost::shared_ptr<ActivationDataAbstract>& adata,
                                                    const bool update_u) {
  // This function computes the derivatives of the cost function based on a Gauss-Newton approximation
  ...
  if (q_dependent_ && v_dependent_) {
    ...
    Eigen::Block<MatrixXs, Eigen::Dynamic, Eigen::Dynamic, true> Rq = rdata->Rx.leftCols(nv);
    cdata->Lx.head(nv).noalias() = Rq.transpose() * adata->Ar;
    rdata->Arr_Rx.leftCols(nv).noalias() = adata->Arr.diagonal().asDiagonal() * Rq;
    cdata->Lxx.topLeftCorner(nv, nv).noalias() = Rq.transpose() * rdata->Arr_Rx.leftCols(nv);
    if (is_ru) {
      cdata->Lxu.topRows(nv).noalias() = Rq.transpose() * rdata->Arr_Ru;
    }
  } else if (v_dependent_) {
    Eigen::Block<MatrixXs, Eigen::Dynamic, Eigen::Dynamic, true> Rv = rdata->Rx.rightCols(nv);
    cdata->Lx.tail(nv).noalias() = Rv.transpose() * adata->Ar;
    rdata->Arr_Rx.rightCols(nv).noalias() = adata->Arr.diagonal().asDiagonal() * Rv;
    cdata->Lxx.bottomRightCorner(nv, nv).noalias() = Rv.transpose() * rdata->Arr_Rx.rightCols(nv);
    if (is_ru) {
