 ...
 * guess defined by setCandidate. Finally solve function is used to define when
 * the search direction and length are computed in each iterate. It also
 * describes the globalization strategy (i.e. regularization) of the numerical
 * optimization.
 *
...
public:
   ...
   * @param[in]  isFeasible  true if the \p init_xs are obtained from
   * integrating the \p init_us (rollout) (default false)
   * @param[in]  regInit     initial guess for the regularization value. Very
   * low values are typical used with very good guess points (init_xs, init_us)
   * @return A boolean that describes if convergence was reached.
   */
  virtual bool
  solve(const std::vector<Eigen::VectorXd> &init_xs = DEFAULT_VECTOR,
   ...
   * \f$(\delta\mathbf{x},\delta\mathbf{u})\f$ for the current guess
   * \f$(\mathbf{x}_s,\mathbf{u}_s)\f$
   *
   * You must call setCandidate first in order to define the current guess. A
   * current guess defines a state and control trajectory
   ...
   * determined step length \f$\alpha\f$; so you need to run first
   * `computeDirection()`. Additionally it returns the cost improvement along
   * the predefined step length.
   *
   ...
   * been reached the local minima. This function is used to evaluate the
   * algorithm convergence. The stopping criteria strictly speaking depends on
   * the search direction (calculated by `computeDirection()`) but it could also
   * depend on the chosen step length, tested by `tryStep()`.
   */
  virtual double stoppingCriteria() = 0;

  /**
   * @brief Return the expected improvement from a given current search
   * direction
   *
   * For computing the expected improvement, you need to compute first the
   * search direction by running `computeDirection()`.
   */
  virtual const Eigen::Vector2d &expectedImprovement() = 0;

  /**
   * @brief Set the solver candidate warm-point values
   * \f$(\mathbf{x}_s,\mathbf{u}_s)\f$
   *
   * The solver candidates are defined as a state and control trajectories
   * \f$(\mathbf{x}_s,\mathbf{u}_s)\f$ of \f$T+1\f$ and \f$T\f$ elements,
   * respectively. Additionally, we need to define is
   * \f$(\mathbf{x}_s,\mathbf{u}_s)\f$ pair is feasible, this means that the
   * dynamics rollout give us produces \f$\mathbf{x}_s\f$.
   *
   ...
   */
  void
  setCandidate(const std::vector<Eigen::VectorXd> &xs_warm = DEFAULT_VECTOR,
