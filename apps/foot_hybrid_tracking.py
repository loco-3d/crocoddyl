...
def createModel(timeStep, footRef, contactName, opPointName):
    ...
    cost1 = CostModelFramePlacement(
        rmodel, nu=actModel.nu, frame=rmodel.getFrameId(opPointName), ref=pinocchio.SE3(eye(3),
                                                                                        np.matrix(footRef).T))
    xweight = np.array([0] * 6 + [0.01] * (rmodel.nv - 6) + [10] * rmodel.nv)
    cost2 = CostModelState(
        rmodel, State, ref=rmodel.defaultState, nu=actModel.nu, activation=ActivationModelWeightedQuad(xweight**2))
...
def createTermModel(timeStep, footRef, contactName, opPointName):
    ...
    termmodel.differential.costs.addCost(
        name='veleff', cost=CostModelFrameVelocity(rmodel, rmodel.getFrameId(opPointName)), weight=10000)
...
termmodel1 = createTermModel(
    timeStep=timeStep, footRef=[.0, FOOTGAP, -1.01927], contactName=rightFrame, opPointName=leftFrame)
models2 = [
    createModel(
        timeStep=timeStep, footRef=[(.2 * k) / T, -FOOTGAP, -1.01927], contactName=leftFrame, opPointName=rightFrame)
    for k in range(T)
]
termmodel2 = createTermModel(
    timeStep=timeStep, footRef=[.2, -FOOTGAP, -1.01927], contactName=leftFrame, opPointName=rightFrame)
