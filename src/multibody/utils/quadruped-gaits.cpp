...
SimpleQuadrupedGaitProblem::SimpleQuadrupedGaitProblem(
    ...
    : rmodel_(rmodel), rdata_(rmodel_),
      ...
      firtstep_(true), defaultstate_(rmodel_.nq + rmodel_.nv) {
...
SimpleQuadrupedGaitProblem::createWalkingProblem(
  ...
  std::vector<boost::shared_ptr<crocoddyl::ActionModelAbstract>> doubleSupport(
      supportknots,
      createSwingFootModel(timestep, support_feet, nullCoM, emptyVector));
...
SimpleQuadrupedGaitProblem::createFootStepModels(
  ...
  for (std::size_t k = 0; k < n_knots; ++k) {
    ...
    for (std::size_t i = 0; i < swingFootIds.size(); ++i) {
      ...
      foot_swing_task.push_back(crocoddyl::FramePlacement(
          swingFootIds[i], pinocchio::SE3(Eigen::Matrix3d::Identity(), tref)));
    }
    ...
    Eigen::Vector3d com_task =
        ...
        com_pos0;
    foot_swing_model.push_back(createSwingFootModel(timestep, support_foot_ids,
                                                    com_task, foot_swing_task));
  }
  // Action model for the foot switch
  foot_swing_model.push_back(
      createFootSwitchModel(support_foot_ids, foot_swing_task));
...
SimpleQuadrupedGaitProblem::createSwingFootModel(
  ...
  for (std::vector<pinocchio::FrameIndex>::const_iterator it =
    ...
    boost::shared_ptr<crocoddyl::ContactModelAbstract> support_contact_model =
        boost::make_shared<crocoddyl::ContactModel3D>(
            state_, xref, actuation_->get_nu(), Eigen::Vector2d(0., 50.));
  ...
  if (com_task.array().allFinite()) {
    boost::shared_ptr<crocoddyl::CostModelAbstract> com_track =
        boost::make_shared<crocoddyl::CostModelCoMPosition>(
            state_, com_task, actuation_->get_nu());
    cost_model->addCost("comTrack", com_track, 1e6);
  }
  if (!foot_swing_task.empty()) {
    for (std::vector<crocoddyl::FramePlacement>::const_iterator it =
             foot_swing_task.begin();
      ...
      boost::shared_ptr<crocoddyl::CostModelAbstract> foot_track =
          boost::make_shared<crocoddyl::CostModelFrameTranslation>(
              state_, xref, actuation_->get_nu());
      cost_model->addCost(rmodel_.frames[it->id].name + "_footTrack",
                          foot_track, 1e6);
    }
  }
  ...
  boost::shared_ptr<crocoddyl::CostModelAbstract> state_reg =
      boost::make_shared<crocoddyl::CostModelState>(
          state_, state_activation, defaultstate_, actuation_->get_nu());
  ...
  if (!foot_swing_task.empty()) {
    for (std::vector<crocoddyl::FramePlacement>::const_iterator it =
      ...
      boost::shared_ptr<crocoddyl::CostModelAbstract> impulse_foot_vel =
          boost::make_shared<crocoddyl::CostModelFrameVelocity>(
              state_, vref, actuation_->get_nu());
      cost_model->addCost(rmodel_.frames[it->id].name + "_footTrack",
                          foot_track, 1e7);
      cost_model->addCost(rmodel_.frames[it->id].name + "_impulseVel",
...
SimpleQuadrupedGaitProblem::createImpulseModel(
  ...
  if (!foot_swing_task.empty()) {
    for (std::vector<crocoddyl::FramePlacement>::const_iterator it =
      ...
      crocoddyl::FrameTranslation xref(it->id, it->placement.translation());
      boost::shared_ptr<crocoddyl::CostModelAbstract> foot_track =
          boost::make_shared<crocoddyl::CostModelFrameTranslation>(state_, xref,
                                                                   0);
      cost_model->addCost(rmodel_.frames[it->id].name + "_footTrack",
                          foot_track, 1e7);
    }
  ...
  boost::shared_ptr<crocoddyl::ActivationModelAbstract> state_activation =
      boost::make_shared<crocoddyl::ActivationModelWeightedQuad>(state_weights);
  boost::shared_ptr<crocoddyl::CostModelAbstract> state_reg =
      boost::make_shared<crocoddyl::CostModelState>(state_, state_activation,
                                                    defaultstate_, 0);
  cost_model->addCost("stateReg", state_reg, 1e1);
