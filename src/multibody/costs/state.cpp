...
CostModelState::CostModelState(boost::shared_ptr<StateMultibody> state,
  ...
  if (static_cast<std::size_t>(xref_.size()) != state_->get_nx()) {
    throw_pretty("Invalid argument: " << "xref has wrong dimension (it should be " + std::to_string(state_->get_nx()) + ")");
  }
  if (activation_->get_nr() != state_->get_ndx()) {
    throw_pretty("Invalid argument: " << "nr is equals to " + std::to_string(state_->get_ndx()));
  }
}
