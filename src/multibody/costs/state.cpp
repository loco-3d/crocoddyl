...
CostModelState::CostModelState(StateMultibody& state, const Eigen::VectorXd& xref, const unsigned int& nu)
...
CostModelState::CostModelState(StateMultibody& state, ActivationModelAbstract& activation, const unsigned int& nu)
    : CostModelAbstract(state, activation, nu), xref_(state.zero()) {
  assert(xref_.size() == state_.get_nx() && "CostModelState: reference is not dimension nx");
  assert(activation_.get_nr() == state_.get_ndx() && "CostModelState: nr is not equals to ndx");
}

CostModelState::CostModelState(StateMultibody& state, const unsigned int& nu)
