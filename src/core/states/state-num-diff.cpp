...
StateNumDiff::StateNumDiff(StateAbstract& state):
  StateAbstract(state.get_nx(), state.get_ndx()),
  state_(state)
{
  disturbance_ = 1e-6;
  // disturbance vector
...
Eigen::VectorXd StateNumDiff::zero() {
  return state_.zero();
}

Eigen::VectorXd StateNumDiff::rand() {
  return state_.rand();
}

void StateNumDiff::diff(const Eigen::Ref<const Eigen::VectorXd>& x0,
                        const Eigen::Ref<const Eigen::VectorXd>& x1,
                        Eigen::Ref<Eigen::VectorXd> dxout) {
  state_.diff(x0, x1, dxout);
}

void StateNumDiff::integrate(const Eigen::Ref<const Eigen::VectorXd>& x,
                             const Eigen::Ref<const Eigen::VectorXd>& dx,
                             Eigen::Ref<Eigen::VectorXd> xout) {
  state_.integrate(x, dx, xout);
}

void StateNumDiff::Jdiff(const Eigen::Ref<const Eigen::VectorXd>& x1,
  ...
  if(firstsecond == Jcomponent::first || firstsecond == Jcomponent::both)
  {
    assert(Jfirst.rows() == ndx_ && Jfirst.cols() == ndx_ &&
           "StateNumDiff::Jdiff: Jfirst must be of the good size");
    Jfirst.setZero();
...
void StateNumDiff::Jintegrate(const Eigen::Ref<const Eigen::VectorXd>& x,
                             const Eigen::Ref<const Eigen::VectorXd>& dx,
                             Eigen::Ref<Eigen::MatrixXd> Jfirst,
                             Eigen::Ref<Eigen::MatrixXd> Jsecond,
                             Jcomponent firstsecond) {
