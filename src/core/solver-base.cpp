...
SolverAbstract::SolverAbstract(boost::shared_ptr<ShootingProblem> problem)
    : problem_(problem), is_feasible_(false), cost_(0.), stop_(0.), xreg_(NAN),
      ureg_(NAN), steplength_(1.), dV_(0.), dVexp_(0.), th_acceptstep_(0.1),
      th_stop_(1e-9), iter_(0) {
...
void SolverAbstract::setCandidate(const std::vector<Eigen::VectorXd> &xs_warm,
  ...
  if (xs_warm.size() == 0) {
    ...
    if (xs_warm.size() != T + 1) {
      throw_pretty("Warm start state has wrong dimension, got "
      ...
      if (static_cast<std::size_t>(xs_warm[t].size()) != nx) {
        throw_pretty("Invalid argument: "
                     << "xs_init[" + std::to_string(t) +
                            "] has wrong dimension (it should be " +
                            std::to_string(nx) + ")");
      }
    }
    const std::size_t &nx =
...
const double &SolverAbstract::get_th_acceptstep() const {
