...
void SolverBoxFDDP::computeGains(const std::size_t &t) {
  ...
  if (nu > 0) {
    ...
    du_lb_.head(nu) =
        problem_->get_runningModels()[t]->get_u_lb() - us_[t].head(nu);
    du_ub_.head(nu) =
        problem_->get_runningModels()[t]->get_u_ub() - us_[t].head(nu);

    const BoxQPSolution &boxqp_sol =
        qp_.solve(Quu_[t].topLeftCorner(nu, nu), Qu_[t].head(nu),
    ...
    K_[t].topRows(nu).noalias() =
        Quu_inv_[t].topLeftCorner(nu, nu) * Qxu_[t].leftCols(nu).transpose();
